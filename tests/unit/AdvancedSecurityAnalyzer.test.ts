...
describe('AdvancedSecurityAnalyzer', () => {
  ...
  describe('Task 2.3 - Advanced Security Analysis (+10 New Threat Types)', () => {
    ...
    it('should analyze cryptographic weaknesses (NEW THREAT TYPE 1)', async () => {
      ...
      expect(result.recommendations.some(rec => rec.includes('cryptographic algorithms'))).toBe(true);
    });
    ...
    it('should analyze data leakage risks (NEW THREAT TYPE 3)', async () => {
      ...
      expect(result.recommendations.some(rec => rec.includes('sensitive data from logs'))).toBe(true);
    });
    ...
    it('should analyze race condition vulnerabilities (NEW THREAT TYPE 6)', async () => {
      ...
      expect(result.issues.some(issue => issue.type === 'race-condition')).toBe(true);
      expect(result.issues.some(issue => issue.type === 'unsafe-shared-state')).toBe(true);
      expect(result.issues.some(issue => issue.type === 'toctou')).toBe(true);
      expect(result.recommendations.some(rec => rec.includes('synchronization'))).toBe(true);
    });
    ...
    it('should analyze CORS and CSP vulnerabilities (NEW THREAT TYPE 8)', async () => {
      ...
      expect(result.issues.some(issue => issue.type === 'insecure-cors')).toBe(true);
      expect(result.issues.some(issue => issue.type === 'missing-csp')).toBe(true);
      expect(result.issues.some(issue => issue.type === 'open-redirect')).toBe(true);
      expect(result.recommendations.some(rec => rec.includes('CORS'))).toBe(true);
    });
