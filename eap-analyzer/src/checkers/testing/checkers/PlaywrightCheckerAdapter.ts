...
/**
 * Информация о конфигурации Playwright
 */
 ...
 */
export class PlaywrightCheckerAdapter extends BaseChecker {
  readonly name = 'playwright-checker';
  readonly category = 'quality' as const;
  readonly description = 'Анализирует конфигурацию и качество Playwright E2E тестов';
   ...
   */
  private async hasPlaywrightDependency(projectPath: string): Promise<boolean> {
    try {
   ...
   */
  private async analyzePlaywrightConfig(projectPath: string): Promise<PlaywrightConfig> {
   ...
   */
  private async parsePlaywrightConfig(
    configPath: string,
    projectPath: string
  ): Promise<Omit<PlaywrightConfig, 'configFound' | 'configPath'>> {
    try {
   ...
   */
  private async countTestFiles(projectPath: string, testDir: string): Promise<number> {
    try {
   ...
   */
  private extractBrowsers(content: string): string[] {
