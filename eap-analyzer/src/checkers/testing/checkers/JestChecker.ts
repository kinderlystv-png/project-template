...
/**
 * Результат анализа Jest конфигурации
 */
export interface JestAnalysisResult {
...
/**
 * Проблема в конфигурации Jest
 */
export interface JestIssue {
  type: 'configuration' | 'performance' | 'coverage' | 'compatibility';
 ...
 */
export class JestChecker extends BaseChecker {
  ...
  async check(project: Project): Promise<CheckResult[]> {
    ...
    try {
      ...
      results.push(
        ResultBuilder.error('jest-analysis-error', 'Ошибка анализа Jest')
   ...
   */
  private async analyzeJest(project: Project): Promise<JestAnalysisResult> {
    const result: JestAnalysisResult = {
      hasConfig: false,
      testFiles: [],
      issues: [],
      recommendations: [],
   ...
   */
  private async findJestConfig(project: Project): Promise<{
    found: boolean;
    path?: string;
    config?: JestConfig;
  }> {
   ...
   */
  private async parseJestConfig(configPath: string): Promise<JestConfig | undefined> {
    try {
   ...
   */
  private async analyzeJestIssues(project: Project, config?: JestConfig): Promise<JestIssue[]> {
    const issues: JestIssue[] = [];
   ...
   */
  private async checkJestPresence(
    project: Project,
    analysis: JestAnalysisResult
  ): Promise<CheckResult> {
    ...
    if (hasJestDependency && hasConfig && hasTests) {
      return ResultBuilder.success('jest-presence', 'Наличие Jest')
      ...
      return ResultBuilder.warning('jest-presence', 'Наличие Jest')
   ...
   */
  private async checkJestConfiguration(analysis: JestAnalysisResult): Promise<CheckResult> {
    ...
    return ResultBuilder.threshold('jest-configuration', 'Конфигурация Jest', configScore, 0.7)
      .message(
  ...
  private async checkTestFiles(
    ...
    const passed = testFilesCount >= TESTING_THRESHOLDS.MIN_TEST_FILES;
    ...
    return ResultBuilder.threshold(
      ...
      'Тестовые файлы Jest',
      testFilesCount,
      TESTING_THRESHOLDS.MIN_TEST_FILES
    )
      .message(`Найдено ${testFilesCount} тестовых файлов Jest`)
      .details({
        totalFiles: testFilesCount,
        threshold: TESTING_THRESHOLDS.MIN_TEST_FILES,
        fileAnalysis,
   ...
   */
  private async checkCoverageSettings(analysis: JestAnalysisResult): Promise<CheckResult> {
    const config = analysis.config!;
    ...
    return ResultBuilder.threshold('jest-coverage', 'Настройки покрытия Jest', coverageScore, 0.6)
   ...
   */
  private async checkPerformanceSettings(analysis: JestAnalysisResult): Promise<CheckResult> {
    const config = analysis.config!;
   ...
   */
  private async checkJestBestPractices(analysis: JestAnalysisResult): Promise<CheckResult> {
    const config = analysis.config!;
    let score = 0;
    ...
    return ResultBuilder.threshold('jest-best-practices', 'Лучшие практики Jest', score, 0.6)
   ...
   */
  private async createOverallJestAssessment(analysis: JestAnalysisResult): Promise<CheckResult> {
    if (!analysis.hasConfig) {
      return ResultBuilder.warning('jest-overall', 'Общая оценка Jest')
    ...
    return ResultBuilder.threshold('jest-overall', 'Общая оценка Jest', overallScore, 0.7)
  ...
  private calculateConfigurationScore(config: JestConfig, issues: JestIssue[]): number {
  ...
  private calculateCoverageScore(config: JestConfig): number {
    let score = 0;
  ...
  private calculatePerformanceScore(config: JestConfig): number {
  ...
  private calculateOverallJestScore(analysis: JestAnalysisResult): number {
    ...
    const testFilesScore = Math.min(
      1,
      analysis.testFiles.length / TESTING_THRESHOLDS.MIN_TEST_FILES
    );
  ...
  private hasOptimalWorkerConfiguration(maxWorkers?: number | string): boolean {
  ...
  private async analyzeTestFileStructure(testFiles: string[]): Promise<{
    totalTests: number;
  ...
  private async hasJestInDependencies(project: Project): Promise<boolean> {
    try {
  ...
  private generateJestRecommendations(analysis: JestAnalysisResult): string[] {
    ...
    if (analysis.testFiles.length < TESTING_THRESHOLDS.MIN_TEST_FILES) {
