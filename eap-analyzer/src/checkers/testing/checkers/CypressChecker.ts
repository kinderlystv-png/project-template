...
/**
 * Информация о Cypress тестовом файле
 */
export interface CypressTestFileInfo {
  /** Путь к файлу */
  path: string;
  /** Тип теста */
  type: 'e2e' | 'integration' | 'component' | 'api';
  /** Количество тестов */
  testCount: number;
  /** Используемые команды */
  commands: string[];
  /** Проблемы в тесте */
  issues: CypressTestIssue[];
  /** Сложность теста */
  complexity: number;
  /** Использует ли Page Object */
  usesPageObject: boolean;
  /** Использует ли пользовательские команды */
  usesCustomCommands: boolean;
}

/**
 * Проблема в Cypress тесте
 */
export interface CypressTestIssue {
  type: 'selector' | 'waiting' | 'hardcoded' | 'performance' | 'best-practice';
 ...
 */
export interface CypressAnalysisResult {
...
/**
 * Проблема настройки Cypress
 */
export interface CypressIssue {
  type: 'configuration' | 'performance' | 'reliability' | 'missing' | 'deprecation';
  severity: SeverityLevel;
  message: string;
  suggestion: string;
  configKey?: string;
 ...
 */
export class CypressChecker extends BaseChecker {
  ...
  async check(project: Project): Promise<CheckResult[]> {
    ...
    try {
      ...
      results.push(
        ResultBuilder.error('cypress-analysis-error', 'Ошибка анализа Cypress')
   ...
   */
  private async analyzeCypress(project: Project): Promise<CypressAnalysisResult> {
    const result: CypressAnalysisResult = {
      hasConfig: false,
      testFiles: [],
      supportFiles: {},
      issues: [],
   ...
   */
  private async findCypressConfig(project: Project): Promise<{
    found: boolean;
    path?: string;
    config?: CypressConfig;
  }> {
   ...
   */
  private async parseCypressConfig(configPath: string): Promise<CypressConfig | undefined> {
    try {
   ...
   */
  private async findCypressTestFiles(
    project: Project,
    config?: CypressConfig
  ): Promise<CypressTestFileInfo[]> {
    const cypressFiles: CypressTestFileInfo[] = [];
   ...
   */
  private async scanForCypressFiles(dirPath: string): Promise<CypressTestFileInfo[]> {
    const cypressFiles: CypressTestFileInfo[] = [];
   ...
   */
  private isCypressTestFile(fileName: string): boolean {
   ...
   */
  private async analyzeCypressTestFile(filePath: string): Promise<CypressTestFileInfo | null> {
    try {
   ...
   * @returns Тип теста
   */
  private determineCypressTestType(content: string): 'e2e' | 'integration' | 'component' | 'api' {
   ...
   */
  private countCypressTests(content: string): number {
   ...
   */
  private calculateCypressComplexity(content: string): number {
    let complexity = 1;
   ...
   */
  private detectPageObjectUsage(content: string): boolean {
   ...
   */
  private detectCustomCommandsUsage(content: string): boolean {
   ...
   */
  private async findCypressTestIssues(content: string): Promise<CypressTestIssue[]> {
    const issues: CypressTestIssue[] = [];
   ...
   */
  private async findCypressSupportFiles(project: Project): Promise<{
    commands?: string;
    index?: string;
    plugins?: string;
  }> {
    const supportFiles: { commands?: string; index?: string; plugins?: string } = {};
    ...
    for (const { type, paths } of supportPaths) {
      for (const supportPath of paths) {
        ...
        try {
          ...
          supportFiles[type as keyof typeof supportFiles] = fullPath;
          break;
        } catch {
          continue;
        }
  ...
  private async checkCypressPresence(
    project: Project,
    analysis: CypressAnalysisResult
  ): Promise<CheckResult> {
    ...
    if (hasCypressDependency && hasConfig && hasTests) {
      return ResultBuilder.success('cypress-presence', 'Наличие Cypress')
      ...
      return ResultBuilder.warning('cypress-presence', 'Наличие Cypress')
  ...
  private async checkCypressConfiguration(analysis: CypressAnalysisResult): Promise<CheckResult> {
  ...
  private async checkCypressTestFiles(
    project: Project,
    analysis: CypressAnalysisResult
  ): Promise<CheckResult> {
  ...
  private async checkCypressSupportFiles(analysis: CypressAnalysisResult): Promise<CheckResult> {
  ...
  private async checkCypressReliability(analysis: CypressAnalysisResult): Promise<CheckResult> {
    const config = analysis.config!;
  ...
  private async checkCypressPerformance(analysis: CypressAnalysisResult): Promise<CheckResult> {
    const config = analysis.config!;
  ...
  private async checkCypressBestPractices(analysis: CypressAnalysisResult): Promise<CheckResult> {
    const config = analysis.config!;
    let score = 0;
    ...
    return ResultBuilder.threshold('cypress-best-practices', 'Лучшие практики Cypress', score, 0.6)
  ...
  private async createCypressSummary(analysis: CypressAnalysisResult): Promise<CheckResult> {
    if (!analysis.hasConfig) {
      return ResultBuilder.warning('cypress-summary', 'Сводка Cypress')
    ...
    return ResultBuilder.threshold('cypress-summary', 'Общая оценка Cypress', overallScore, 0.7)
      .message(
  ...
  private async analyzeCypressIssues(
    project: Project,
    config?: CypressConfig
  ): Promise<CypressIssue[]> {
    const issues: CypressIssue[] = [];
  ...
  private calculateCypressConfigurationScore(
    config: CypressConfig,
    issues: CypressIssue[]
  ): number {
  ...
  private calculateSupportFilesScore(supportFiles: {
    commands?: string;
    index?: string;
    plugins?: string;
  }): number {
  ...
  private calculateOverallCypressScore(analysis: CypressAnalysisResult): number {
  ...
  private async hasCypressInDependencies(project: Project): Promise<boolean> {
    try {
  ...
  private getCypressTestFileTypes(testFiles: CypressTestFileInfo[]): Record<string, number> {
  ...
  private generateCypressRecommendations(analysis: CypressAnalysisResult): string[] {
