...
/**
 * Общая статистика E2E тестирования
 */
export interface E2ETestingStats {
  /** Общее количество E2E тестов */
  totalTests: number;
  /** Количество тестовых файлов */
  totalFiles: number;
  /** Покрытие критических путей */
  criticalPathsCoverage: number;
  /** Средняя сложность тестов */
...
/**
 * Результат анализа E2E тестирования
 */
export interface E2ETestingAnalysisResult {
  /** Обнаруженные фреймворки */
  frameworks: E2EFrameworkInfo[];
  /** Основной фреймворк */
  primaryFramework?: E2EFramework;
  /** Статистика */
  stats: E2ETestingStats;
  /** Проблемы */
  issues: E2ETestingIssue[];
  /** Рекомендации */
 ...
 */
export interface E2ETestingIssue {
  type: 'framework' | 'coverage' | 'quality' | 'performance' | 'maintenance';
 ...
 */
export class E2ETestingChecker extends BaseChecker {
  private testFileFinder: TestFileFinder;
  private playwrightChecker: PlaywrightChecker;
  private cypressChecker: CypressChecker;
  ...
  constructor() {
    ...
    this.playwrightChecker = new PlaywrightChecker();
    this.cypressChecker = new CypressChecker();
  }
  ...
  async check(project: Project): Promise<CheckResult[]> {
    ...
    try {
      ...
      results.push(
        ResultBuilder.error('e2e-testing-analysis-error', 'Ошибка анализа E2E тестирования')
   ...
   */
  private async analyzeE2ETesting(project: Project): Promise<E2ETestingAnalysisResult> {
    const result: E2ETestingAnalysisResult = {
      frameworks: [],
      stats: {
        totalTests: 0,
        totalFiles: 0,
   ...
   */
  private async detectE2EFrameworks(project: Project): Promise<E2EFrameworkInfo[]> {
    const frameworks: E2EFrameworkInfo[] = [];
  ...
  private async analyzeFramework(
    project: Project,
    framework: E2EFramework
  ): Promise<E2EFrameworkInfo> {
    const info: E2EFrameworkInfo = {
      name: framework,
      installed: false,
      configured: false,
      testCount: 0,
   ...
   */
  private async isFrameworkInstalled(project: Project, framework: E2EFramework): Promise<boolean> {
    try {
   ...
   */
  private async findFrameworkConfig(
    project: Project,
    framework: E2EFramework
  ): Promise<{
    found: boolean;
    ...
    const configFiles: Record<E2EFramework, string[]> = {
   ...
   */
  private async countFrameworkTests(project: Project, framework: E2EFramework): Promise<number> {
    try {
      const testDirs: Record<E2EFramework, string[]> = {
      ...
      const patterns: Record<E2EFramework, RegExp[]> = {
   ...
   * @returns Количество тестов
   */
  private async countTestsInDirectory(dirPath: string, patterns: RegExp[]): Promise<number> {
    let count = 0;
  ...
  private async calculateFrameworkScore(
    ...
    if (testCount >= TESTING_THRESHOLDS.MIN_E2E_TESTS) {
      score += 0.4;
    } else if (testCount > 0) {
      score += 0.2;
    }
   ...
   */
  private determinePrimaryFramework(frameworks: E2EFrameworkInfo[]): E2EFramework | undefined {
   ...
   * @returns Статистика
   */
  private async calculateE2EStats(
    project: Project,
    frameworks: E2EFrameworkInfo[]
  ): Promise<E2ETestingStats> {
    const stats: E2ETestingStats = {
      totalTests: 0,
      totalFiles: 0,
      criticalPathsCoverage: 0,
      averageComplexity: 0,
  ...
  private async checkE2ETestingPresence(
    project: Project,
    analysis: E2ETestingAnalysisResult
  ): Promise<CheckResult> {
    ...
    if (hasE2ETesting && totalTests > 0) {
      return ResultBuilder.success('e2e-testing-presence', 'Наличие E2E тестирования')
      ...
      return ResultBuilder.warning('e2e-testing-presence', 'Наличие E2E тестирования')
  ...
  private async checkE2EFrameworkSelection(
    analysis: E2ETestingAnalysisResult
  ): Promise<CheckResult> {
    const installedFrameworks = analysis.frameworks.filter(f => f.installed);
    ...
    if (installedFrameworks.length === 1) {
      ...
      return ResultBuilder.success('e2e-framework-selection', 'Выбор E2E фреймворка')
      ...
      return ResultBuilder.warning('e2e-framework-selection', 'Выбор E2E фреймворка')
      ...
      return ResultBuilder.error('e2e-framework-selection', 'Выбор E2E фреймворка')
  ...
  private async checkE2ETestCoverage(
    project: Project,
    analysis: E2ETestingAnalysisResult
  ): Promise<CheckResult> {
    const totalTests = analysis.stats.totalTests;
    const criticalPathsCoverage = analysis.stats.criticalPathsCoverage;
    const passed = totalTests >= TESTING_THRESHOLDS.MIN_E2E_TESTS && criticalPathsCoverage >= 0.6;
  ...
  private async checkE2ETestQuality(analysis: E2ETestingAnalysisResult): Promise<CheckResult> {
  ...
  private async checkE2EPerformance(analysis: E2ETestingAnalysisResult): Promise<CheckResult> {
  ...
  private async checkE2EMaintainability(analysis: E2ETestingAnalysisResult): Promise<CheckResult> {
  ...
  private async checkE2EBestPractices(analysis: E2ETestingAnalysisResult): Promise<CheckResult> {
    const bestPracticesScore = analysis.stats.bestPracticesScore;
    const installedFrameworks = analysis.frameworks.filter(f => f.installed);
    ...
    if (analysis.stats.totalTests >= TESTING_THRESHOLDS.MIN_E2E_TESTS) {
  ...
  private async checkMultipleFrameworks(analysis: E2ETestingAnalysisResult): Promise<CheckResult> {
    const installedFrameworks = analysis.frameworks.filter(f => f.installed);

    return ResultBuilder.warning('e2e-multiple-frameworks', 'Множественные E2E фреймворки')
      .message(`Установлено ${installedFrameworks.length} E2E фреймворков`)
      .details({
        frameworks: installedFrameworks.map(f => ({
          name: f.name,
  ...
  private async createE2ETestingSummary(analysis: E2ETestingAnalysisResult): Promise<CheckResult> {
    const installedFrameworks = analysis.frameworks.filter(f => f.installed);

    if (installedFrameworks.length === 0) {
      return ResultBuilder.warning('e2e-testing-summary', 'Сводка E2E тестирования')
        .message('E2E тестирование не настроено')
        .details({
          configured: false,
    ...
    return ResultBuilder.threshold(
      ...
      .details({
        ...
        breakdown: {
          ...
          coverage: Math.min(1, analysis.stats.totalTests / TESTING_THRESHOLDS.MIN_E2E_TESTS),
  ...
  private async analyzeE2EIssues(
    project: Project,
    frameworks: E2EFrameworkInfo[]
  ): Promise<E2ETestingIssue[]> {
    const issues: E2ETestingIssue[] = [];
  ...
  private calculateOverallE2EScore(analysis: E2ETestingAnalysisResult): number {
    const installedFrameworks = analysis.frameworks.filter(f => f.installed);
    ...
    const frameworksScore =
      installedFrameworks.reduce((sum, f) => sum + f.score, 0) / installedFrameworks.length;
    const coverageScore = Math.min(1, analysis.stats.totalTests / TESTING_THRESHOLDS.MIN_E2E_TESTS);
  ...
  private async countE2ETestFiles(project: Project): Promise<number> {
  ...
  private async estimateCriticalPathsCoverage(project: Project): Promise<number> {
  ...
  private async calculateAverageTestComplexity(project: Project): Promise<number> {
  ...
  private calculateBestPracticesScore(frameworks: E2EFrameworkInfo[]): number {
    const installedFrameworks = frameworks.filter(f => f.installed);
  ...
  private generateE2ERecommendations(analysis: E2ETestingAnalysisResult): string[] {
