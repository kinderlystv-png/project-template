 ...
 */
export class E2EChecker extends BaseChecker {
  constructor() {
    super(
      'E2EChecker',
      AnalysisCategory.TESTING,
      'Проверка настроек E2E тестирования',
    ...
    try {
      ...
      results.push(
        this.createResult(
          'e2e-check-error',
          'E2E Check Error',
          'Ошибка при проверке E2E тестирования',
          false,
          `Ошибка: ${errorMessage}`,
          0,
          10,
    ...
    return [
      {
        ...
        maxScore: 30,
        tags: [AnalysisCategory.TESTING, 'e2e', 'playwright'],
      },
      {
        id: 'cypress-e2e',
        ...
        severity: SeverityLevel.MEDIUM,
        maxScore: 30,
        tags: [AnalysisCategory.TESTING, 'e2e', 'cypress'],
      },
      {
        id: 'e2e-structure',
        ...
        maxScore: 20,
        tags: [AnalysisCategory.TESTING, 'e2e', 'structure'],
      },
    ];
  }
  ...
  async isApplicable(project: Project): Promise<boolean> {
    try {
      ...
      if (hasE2EFramework) return true;

      // Проверяем наличие конфигурационных файлов E2E
      const hasE2EConfig =
        (await project.exists('playwright.config.ts')) ||
   ...
   */
  private async analyzeE2EFrameworks(project: Project): Promise<E2EFrameworkInfo[]> {
   ...
   */
  private async analyzePlaywright(
    project: Project,
    files: string[],
    ...
    const testFiles = files.filter(
      file =>
        (file.includes('e2e') || file.includes('tests')) &&
  ...
  private async analyzeCypress(
    ...
    const testFiles = files.filter(
      file =>
        (file.includes('cypress') || file.includes('e2e')) &&
   ...
   */
  private async analyzeWebdriverIO(
    project: Project,
    files: string[],
    deps: Record<string, string>
  ): Promise<E2EFrameworkInfo> {
    ...
    const testFiles = files.filter(
      file =>
        (file.includes('e2e') || file.includes('test')) &&
   ...
   */
  private async analyzePuppeteer(
    project: Project,
    files: string[],
    deps: Record<string, string>
  ): Promise<E2EFrameworkInfo> {
   ...
   */
  private async checkE2EStructure(project: Project): Promise<CheckResult> {
    const files = await project.getFileList();
    ...
    const e2eDirs = [
      ...new Set(e2eFiles.map(file => file.split('/').slice(0, -1).join('/'))),
    ].filter(dir => dir);
    ...
    if (e2eFiles.length === 0) {
      ...
      if (e2eFiles.length < 3) {
        ...
        recommendations.push(
          'Добавьте больше E2E тестов для покрытия основных пользовательских путей'
        );
      }
    ...
    const hasE2EHelpers = files.some(
      file =>
        (file.includes('e2e') || file.includes('test')) &&
    ...
    return this.createResult(
      'e2e-structure',
      'E2E Test Structure Check',
      'Проверка структуры E2E тестов',
      score >= 15,
      `E2E структура: ${e2eFiles.length} тестов в ${e2eDirs.length} директориях`,
      Math.max(0, score),
      20,
      score >= 15 ? SeverityLevel.LOW : SeverityLevel.MEDIUM,
      {
   ...
   */
  private createE2EFrameworkResult(framework: E2EFrameworkInfo): CheckResult {
    const issues: string[] = [];
    let score = 100;
    const recommendations: string[] = [];
    ...
    return this.createResult(
      `${framework.name.toLowerCase()}-e2e`,
      `${framework.name} E2E Check`,
      `Проверка настройки ${framework.name} для E2E тестирования`,
      score >= 70,
      `${framework.name}: ${framework.testFiles.length} E2E тестов, конфигурация ${framework.configured ? 'найдена' : 'не найдена'}`,
      Math.max(0, score),
      100,
   ...
   */
  private createNoE2EFrameworkResult(): CheckResult {
    return this.createResult(
      'no-e2e-framework',
      'No E2E Framework',
      'E2E фреймворк не обнаружен',
      false,
      'В проекте не найдено E2E тестовых фреймворков',
      0,
      100,
      SeverityLevel.MEDIUM,
      {
   ...
   */
  private hasAnyE2EFramework(dependencies: Record<string, string>): boolean {
    const e2eFrameworks = [
      'playwright',
      '@playwright/test',
      'cypress',
