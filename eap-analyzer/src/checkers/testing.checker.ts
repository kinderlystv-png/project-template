...
export class TestingChecker extends BaseChecker {
  readonly name = 'Testing Setup Check';
  readonly category = 'quality' as const;
  readonly description = 'Проверка настройки тестирования и покрытия';
  ...
  private async checkTestTypes(
    ...
    try {
      ...
      if (totalTests === 0) {
        ...
        console.log(
          `Найдено тестов: unit=${foundTests.unit}, integration=${foundTests.integration}, component=${foundTests.component}`
        );
      }
    } catch {
  ...
  private async checkCIIntegration(
    projectPath: string,
    issues: string[],
    recommendations: string[]
  ): Promise<void> {
