...
export class CodeQualityChecker {
  ...
  private async checkESLintConfig(): Promise<CheckResult> {
    const configFiles = [
      '.eslintrc.js',
      '.eslintrc.cjs',
      '.eslintrc.json',
    ...
    if (passed) {
      try {
        ...
        const hasSvelte =
          content.includes('@typescript-eslint') || content.includes('eslint-plugin-svelte');
        ...
        if (hasSvelte) details += '. Svelte правила настроены';
        if (hasTypeScript) details += '. TypeScript правила настроены';
        if (hasStrict) details += '. Строгие правила активны';
    ...
    return {
      check: {
        ...
        category: 'code-quality' as CheckCategory,
        score: 20,
        critical: true,
  ...
  private async checkPrettierConfig(): Promise<CheckResult> {
    const configFiles = [
      '.prettierrc',
      '.prettierrc.json',
      '.prettierrc.js',
    ...
    for (const file of configFiles) {
      if (existsSync(join(this.context.projectPath, configFile))) {
        passed = true;
        configFile = file;
        break;
      }
    ...
    if (!passed && existsSync(packagePath)) {
      try {
    ...
    if (passed) {
      ...
      const ignorePath = join(this.context.projectPath, '.prettierignore');
  ...
  private async checkTypeScriptConfig(): Promise<CheckResult> {
    const tsconfigPath = join(this.context.projectPath, 'tsconfig.json');
  ...
  private async checkCommitLintConfig(): Promise<CheckResult> {
    const configFiles = [
  ...
  private async checkEditorConfig(): Promise<CheckResult> {
    const editorConfigPath = join(this.context.projectPath, '.editorconfig');
    const passed = existsSync(editorConfigPath);
    ...
    if (passed) {
      try {
        ...
        let configCount = 0;
        if (hasRoot) configCount++;
        if (hasIndent) configCount++;
        if (hasCharset) configCount++;
        if (hasEndOfLine) configCount++;
  ...
  private async checkLintingScripts(): Promise<CheckResult> {
    const packagePath = join(this.context.projectPath, 'package.json');
    let passed = false;
    let details = '';
    ...
    if (existsSync(packagePath)) {
      try {
        ...
        if (hasLint || hasLintFix || hasTypeCheck) {
          ...
          if (hasLint) details += '. lint';
          if (hasLintFix) details += '. lint:fix';
          if (hasTypeCheck) details += '. type-check';
  ...
  private async checkFormattingScripts(): Promise<CheckResult> {
    const packagePath = join(this.context.projectPath, 'package.json');
    let passed = false;
    let details = '';
    ...
    if (existsSync(packagePath)) {
      try {
        ...
        if (hasFormat || hasFormatCheck) {
          ...
          if (hasFormat) details += '. format';
          if (hasFormatCheck) details += '. format:check';
  ...
  private async checkPreCommitHooks(): Promise<CheckResult> {
      ...
      try {
        ...
        if (hasLintStaged) {
          ...
          if (pkg['lint-staged']) {
            ...
            if (hasTypeScript) configuredTypes++;
            if (hasSvelte) configuredTypes++;
            if (hasJSON) configuredTypes++;
  ...
  private async checkCodeStyleConsistency(): Promise<CheckResult> {
