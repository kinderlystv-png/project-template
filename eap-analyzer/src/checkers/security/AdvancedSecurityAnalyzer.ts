...
interface ISecurityAnalyzer {
  ...
  analyze(projectPath: string): Promise<SecurityResult>;
}
...
export class AdvancedSecurityAnalyzer implements ISecurityAnalyzer {
  ...
  async analyze(projectPath: string): Promise<SecurityResult> {
    const startTime = Date.now();
    let score = 100;
    const issues: Array<{ severity: string; message: string; type: string }> = [];
    ...
    if (!fs.existsSync(projectPath)) {
      return {
        ...
        issues: [{ severity: 'high', message: `Security analysis failed: Project path does not exist: ${projectPath}`, type: 'analysis-error' }],
    ...
    try {
      ...
      return {
        ...
        issues: [{ severity: 'high', message: `Security analysis failed: ${error}`, type: 'analysis-error' }],
        recommendations: ['Fix security analysis errors before deployment'],
        analysisTime: Date.now() - startTime,
        details: {
          analyzer: this.name,
   ...
   */
  private async analyzeCryptographicWeaknesses(projectPath: string): Promise<{
    metrics: Record<string, unknown>;
    penalty: number;
    issues: Array<{ severity: string; message: string; type: string }>;
    recommendations: string[];
    ...
    try {
      ...
      if (weakCryptoAlgorithms > 0) {
        recommendations.push('Replace weak cryptographic algorithms (DES, RC4, MD5, SHA1) with strong alternatives (AES, SHA-256)');
      }
      if (hardcodedCryptoKeys > 0) {
        recommendations.push('Move cryptographic keys to secure environment variables or key management systems');
      }
      if (insecureRandomness > 0) {
        recommendations.push('Use crypto.randomBytes() instead of Math.random() for cryptographic purposes');
      }
      ...
      return {
        metrics,
        penalty: 5,
        issues: [{ severity: 'medium', message: `Crypto analysis failed: ${error}`, type: 'analysis-error' }],
   ...
   */
  private async analyzeAuthenticationFlaws(projectPath: string): Promise<{
    metrics: Record<string, unknown>;
    penalty: number;
    issues: Array<{ severity: string; message: string; type: string }>;
    recommendations: string[];
    ...
    try {
      ...
      for (const filePath of codeFiles) {
        ...
        if (content.includes('login') || content.includes('auth')) {
          const mfaPatterns = [
            /mfa|2fa|two.factor|totp|authenticator/gi
          ];
      ...
      if (insecureSessionManagement > 0) {
        recommendations.push('Use secure session management: httpOnly cookies, regenerate session IDs, secure storage');
      }
      if (missingMFA > 0) {
        recommendations.push('Implement multi-factor authentication for enhanced security');
      }
      if (insecureTokens > 0) {
        recommendations.push('Use cryptographically secure random token generation and strong JWT secrets');
      }
      ...
      return {
        metrics,
        penalty: 3,
        issues: [{ severity: 'low', message: `Authentication analysis failed: ${error}`, type: 'analysis-error' }],
   ...
   */
  private async analyzeDataLeakageRisks(projectPath: string): Promise<{
    metrics: Record<string, unknown>;
    penalty: number;
    issues: Array<{ severity: string; message: string; type: string }>;
    recommendations: string[];
    ...
    try {
      ...
      if (sensitiveDataInLogs > 0) {
        recommendations.push('Remove sensitive data from logs and implement proper log sanitization');
      }
      if (dataInComments > 0) {
      ...
      return {
        metrics,
        penalty: 2,
        issues: [{ severity: 'low', message: `Data leakage analysis failed: ${error}`, type: 'analysis-error' }],
   ...
   */
  private async analyzeContainerSecurity(projectPath: string): Promise<{
    metrics: Record<string, unknown>;
    penalty: number;
    issues: Array<{ severity: string; message: string; type: string }>;
    recommendations: string[];
    ...
    try {
      ...
      for (const composeFile of composeFiles) {
        ...
        if (fs.existsSync(composePath)) {
          ...
          const composeSecurityIssues = [
            ...
            { pattern: /security_opt.*apparmor:unconfined/gi, issue: 'Disabled AppArmor', severity: 'high' }
          ];
      ...
      if (metrics.dockerInsecurePractices) {
        recommendations.push('Fix Docker security issues: use specific tags, avoid curl|sh, use COPY instead of ADD');
      }
      ...
      if (metrics.secretsInDockerfile) {
        recommendations.push('Remove secrets from Dockerfile and use Docker secrets or environment variables');
      }
      ...
      return {
        metrics,
        penalty: 1,
        issues: [{ severity: 'low', message: `Container security analysis failed: ${error}`, type: 'analysis-error' }],
   ...
   */
  private async analyzeDeserializationVulns(projectPath: string): Promise<{
    metrics: Record<string, unknown>;
    penalty: number;
    issues: Array<{ severity: string; message: string; type: string }>;
    recommendations: string[];
    ...
    try {
      ...
      if (unsafeDeserialization > 0) {
        recommendations.push('Validate and sanitize all input before deserialization, use safe parsing libraries');
      }
      if (dangerousEvals > 0) {
        recommendations.push('Avoid eval() and Function() constructor, use safer alternatives like JSON.parse()');
      }
      if (unsafeJSONParsing > 0) {
        recommendations.push('Implement input validation and use try-catch blocks for JSON parsing');
      }
      ...
      return {
        metrics,
        penalty: 3,
        issues: [{ severity: 'medium', message: `Deserialization analysis failed: ${error}`, type: 'analysis-error' }],
   ...
   */
  private async analyzeRaceConditions(projectPath: string): Promise<{
    metrics: Record<string, unknown>;
    penalty: number;
    issues: Array<{ severity: string; message: string; type: string }>;
    recommendations: string[];
    ...
    try {
      ...
      if (raceConditionRisks > 0) {
        recommendations.push('Use proper synchronization mechanisms and avoid check-then-act patterns');
      }
      if (unsafeSharedState > 0) {
        recommendations.push('Implement proper locking or use immutable data structures for shared state');
      }
      if (timeOfCheckTimeOfUse > 0) {
        recommendations.push('Use atomic operations or proper file locking to prevent TOCTOU vulnerabilities');
      }
      ...
      return {
        metrics,
        penalty: 2,
        issues: [{ severity: 'low', message: `Race condition analysis failed: ${error}`, type: 'analysis-error' }],
   ...
   */
  private async analyzeCommandInjection(projectPath: string): Promise<{
    metrics: Record<string, unknown>;
    penalty: number;
    issues: Array<{ severity: string; message: string; type: string }>;
    recommendations: string[];
    ...
    try {
      ...
      if (commandInjectionRisks > 0) {
        recommendations.push('Use parameterized commands and avoid string concatenation in exec() calls');
      }
      if (unsafeShellExecution > 0) {
        recommendations.push('Disable shell execution and use direct command execution with proper arguments');
      }
      if (pathInjection > 0) {
        recommendations.push('Validate and sanitize all file paths, use path.resolve() and path.normalize()');
      }
      ...
      return {
        metrics,
        penalty: 4,
        issues: [{ severity: 'medium', message: `Command injection analysis failed: ${error}`, type: 'analysis-error' }],
   ...
   */
  private async analyzeCORSAndCSP(projectPath: string): Promise<{
    metrics: Record<string, unknown>;
    penalty: number;
    issues: Array<{ severity: string; message: string; type: string }>;
    recommendations: string[];
    ...
    try {
      ...
      for (const filePath of allFiles) {
        ...
        if (filePath.endsWith('.html') || filePath.includes('index.')) {
          ...
          if (!hasCSP && content.includes('<head>')) {  // Проверяю что это действительно HTML
            missingCSP++;
            penalty += 5;
            issues.push({
              severity: 'medium',
      ...
      return {
        metrics,
        penalty: 2,
        issues: [{ severity: 'low', message: `CORS/CSP analysis failed: ${error}`, type: 'analysis-error' }],
   ...
   */
  private async analyzePathTraversal(projectPath: string): Promise<{
    metrics: Record<string, unknown>;
    penalty: number;
    issues: Array<{ severity: string; message: string; type: string }>;
    recommendations: string[];
    ...
    try {
      ...
      if (pathTraversalRisks > 0) {
        recommendations.push('Validate and normalize all file paths, use path.resolve() and prevent .. sequences');
      }
      if (unsafeFileOperations > 0) {
        recommendations.push('Sanitize user input before file operations and use whitelisting for allowed paths');
      }
      ...
      return {
        metrics,
        penalty: 3,
        issues: [{ severity: 'medium', message: `Path traversal analysis failed: ${error}`, type: 'analysis-error' }],
   ...
   */
  private async analyzeInsecureLogging(projectPath: string): Promise<{
    metrics: Record<string, unknown>;
    penalty: number;
    issues: Array<{ severity: string; message: string; type: string }>;
    recommendations: string[];
    ...
    try {
      ...
      return {
        metrics,
        penalty: 2,
        issues: [{ severity: 'low', message: `Logging security analysis failed: ${error}`, type: 'analysis-error' }],
  ...
  private async findCodeFiles(projectPath: string): Promise<string[]> {
    ...
    const extensions = ['.js', '.jsx', '.ts', '.tsx', '.vue', '.svelte', '.py', '.php', '.go', '.rb', '.java'];
    ...
    const searchDirectory = (dirPath: string) => {
      ...
      for (const entry of entries) {
        ...
        if (entry.isDirectory() && !['node_modules', '.git', '.svelte-kit', 'dist', 'build'].includes(entry.name)) {
          searchDirectory(fullPath);
