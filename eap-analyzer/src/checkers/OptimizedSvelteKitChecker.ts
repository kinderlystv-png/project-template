...
interface FileCheckConfig {
  ...
  level: CheckLevel;
  tags: string[];
  validators?: Array<(_fileContent: string) => ValidationResult>;
...
abstract class BaseFileChecker {
   ...
   */
  protected getPackageJson(): { exists: boolean; data?: Record<string, unknown>; error?: string } {
  ...
  protected createContentValidator(
    ...
    failMessage: string,
    isWarning = false
  ): (fileContent: string) => ValidationResult {
    return (fileContent: string) => {
      const passed =
        typeof pattern === 'string' ? fileContent.includes(pattern) : pattern.test(fileContent);
