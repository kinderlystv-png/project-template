...
export class CodeQualityChecker extends BaseChecker {
  readonly name = 'Code Quality Check';
  readonly category = 'quality' as const;
  readonly description = 'Проверка общих метрик качества кода';
  ...
  private async checkLinterConfig(
    projectPath: string,
    issues: string[],
  ...
  private async checkTestingSetup(
    projectPath: string,
    issues: string[],
    recommendations: string[]
  ): Promise<void> {
    ...
    try {
      ...
      const hasTestScript = Object.keys(scripts).some(
  ...
  private async checkDocumentation(
    ...
    // Проверка LICENSE
    if (
      !this.fileExists(path.join(projectPath, 'LICENSE')) &&
  ...
  private async hasTypeScriptFiles(projectPath: string): Promise<boolean> {
    try {
  ...
  private async getFiles(projectPath: string, patterns: string[]): Promise<string[]> {
