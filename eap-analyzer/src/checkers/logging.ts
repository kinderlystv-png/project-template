...
export class LoggingChecker {
  ...
  private async checkLoggingLibraries(): Promise<CheckResult> {
    ...
    if (existsSync(packagePath)) {
      try {
        ...
        if (foundLibraries.length > 0) {
          ...
          const hasStructuredLogging = foundLibraries.some(lib =>
            ['winston', 'pino', 'bunyan'].includes(lib)
          );
    ...
    return {
      check: {
        ...
        category: 'logging' as CheckCategory,
        score: 20,
        critical: false,
  ...
  private async checkLoggingConfiguration(): Promise<CheckResult> {
    const configFiles = [
      'winston.config.js',
      'winston.config.ts',
      'logger.config.js',
    ...
    if (passed) {
      ...
      for (const file of mainFiles) {
        if (existsSync(join(this.context.projectPath, file))) {
          ...
          try {
            const content = readFileSync(join(this.context.projectPath, file), 'utf-8');
  ...
  private async checkLogLevels(): Promise<CheckResult> {
    let passed = false;
    let details = '';
    ...
    try {
      ...
      const loggerFile = join(this.context.projectPath, 'src', 'lib', 'logger.ts');
      const loggerFileJs = join(this.context.projectPath, 'src', 'lib', 'logger.js');

      if (existsSync(loggerFile) || existsSync(loggerFileJs)) {
        const filePath = existsSync(loggerFile) ? loggerFile : loggerFileJs;
  ...
  private async checkConsoleUsage(): Promise<CheckResult> {
    let passed = false;
    let details = '';

    try {
      // Проверим использование console.* в исходном коде
      const srcPath = join(this.context.projectPath, 'src');
      ...
      if (existsSync(srcPath)) {
        ...
        if (existsSync(packagePath)) {
          ...
          if (hasLintScript && hasLintScript.includes('eslint')) {
            ...
            for (const eslintFile of eslintFiles) {
              const eslintPath = join(this.context.projectPath, eslintFile);
        ...
        if (!passed) {
          ...
          const loggerExists =
            existsSync(join(srcPath, 'lib', 'logger.ts')) ||
            existsSync(join(srcPath, 'lib', 'logger.js')) ||
            existsSync(join(srcPath, 'utils', 'logger.ts'));
  ...
  private async checkProductionLogging(): Promise<CheckResult> {
    let passed = false;
    let details = '';
        ...
        if (existsSync(filePath)) {
          try {
            ...
            if (content.includes('NODE_ENV') && content.includes('production')) {
              ...
              const hasFileTransport =
                content.includes('File') || content.includes('DailyRotateFile');
              const hasLogRotation =
              ...
              const hasErrorOnlyInProd =
                content.includes('error') && content.includes('production');

              details = `Production логирование настроено в ${foundFile}`;

              if (hasFileTransport) details += '. Файловое логирование';
              if (hasLogRotation) details += '. Ротация логов';
              if (hasJsonFormat) details += '. JSON формат';
              if (hasErrorOnlyInProd) details += '. Фильтрация по уровням';
