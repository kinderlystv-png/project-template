...
import { SecurityRecommendation } from './RecommendationEngine.js';

export class ConfigFixTemplates {
  /**
...
${this.getCORSAfterExample()}`,
  ...
  static generateEnvExposureFix(envData: {
    file: string;
    exposedVars: string[];
    framework: 'vite' | 'next' | 'webpack' | 'generic';
  }): SecurityRecommendation {
  ...
  private static getCORSBeforeExample(currentConfig: any): string {
  ...
  private static getCORSAfterExample(): string {
...
app.use(cors({
  ...
  private static getCORSImplementationGuide(): string {
...
app.post('/api/secure', cors(corsOptions), authMiddleware, handler);`;
  ...
  private static getEnvExposureBefore(exposedVars: string[]): string {
    return `// НЕБЕЗОПАСНО ❌:
const config = {
...
export default config;`;
  ...
  private static getEnvExposureAfter(framework: string): string {
...
module.exports = {
  ...
  private static getEnvExposureGuide(framework: string): string {
...
const dbUrl = process.env.DATABASE_URL;         // ✅ Работает`
  ...
  private static getSecurityHeadersBefore(): string {
    return `// НЕБЕЗОПАСНО ❌:
app.get('/', (req, res) => {
...
app.use(express.static('public')); // Статичные файлы без защиты`;
  ...
  private static getSecurityHeadersAfter(): string {
    return `// БЕЗОПАСНО ✅:
...
app.use((req, res, next) => {
  ...
  private static getSecurityHeadersGuide(missingHeaders: string[]): string {
    const headerGuide: Record<string, string> = {
...
app.use(helmet({
  ${missingHeaders.includes('Content-Security-Policy') ? `contentSecurityPolicy: {
    directives: {
      defaultSrc: ["'self'"],
  ...
  ${missingHeaders.includes('Strict-Transport-Security') ? `hsts: {
  ...
  private static getDockerSecurityBefore(issues: string[]): string {
    return `# НЕБЕЗОПАСНО ❌:
...
CMD ["node", "index.js"]`;
  ...
  private static getDockerSecurityAfter(): string {
    return `# БЕЗОПАСНО ✅:
...
CMD ["node", "index.js"]`;
  ...
  private static getDockerSecurityGuide(): string {
...
services:
  ...
  private static getFrameworkEnvDocs(framework: string): string {
