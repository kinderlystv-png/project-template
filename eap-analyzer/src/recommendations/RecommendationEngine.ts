...
import {
  ...
  type WebSecurityRecommendation,
} from '../checkers/security/WebSecurityFixTemplates.js';
import type { WebSecurityResult } from '../checkers/security/WebSecurityChecker.js';
...
export interface SecurityRecommendation {
  ...
  category: 'dependencies' | 'code' | 'config' | 'xss' | 'csrf' | 'injection';
  priority?: number; // Добавляем для совместимости с веб-рекомендациями
  fixTemplate: {
    steps: string[];
    codeExample?: string;
    beforeCode?: string;
  ...
  tags?: string[]; // Добавляем для веб-рекомендаций
}
...
export class RecommendationEngine {
   ...
   */
  static generateRecommendation(issueContext: IssueContext): SecurityRecommendation {
   ...
   */
  private static generateDependencyRecommendation(context: IssueContext): SecurityRecommendation {
...
"overrides": {
   ...
   */
  private static generateCodeSecurityRecommendation(context: IssueContext): SecurityRecommendation {
...
API_KEY=your_secret_key_here`,
   ...
   */
  private static generateConfigRecommendation(context: IssueContext): SecurityRecommendation {
    const { details } = context;
...
const privateConfig = {
   ...
   */
  private static generateGeneralRecommendation(context: IssueContext): SecurityRecommendation {
    return {
      id: 'general-security',
   ...
   */
  private static generateWebSecurityRecommendation(context: IssueContext): SecurityRecommendation {
    const { details, type } = context;

    // Конвертируем из WebSecurityRecommendation в SecurityRecommendation
    if (details.webRecommendation) {
      const webRec = details.webRecommendation as WebSecurityRecommendation;
   ...
   */
  private static mapWebDifficulty(estimatedTime: string): 'easy' | 'medium' | 'hard' {
   ...
   */
  public static generateWebRecommendations(webResult: WebSecurityResult): SecurityRecommendation[] {
    ...
    webRecommendations.forEach((webRec: WebSecurityRecommendation) => {
      const securityRec: SecurityRecommendation = {
        id: webRec.id,
        title: webRec.title,
        description: webRec.description,
        severity: webRec.severity,
        ...
        documentation: {
          links: webRec.resources.map((r: any) => r.url),
          explanation: webRec.description,
        },
        estimatedTime: webRec.estimatedTime,
        difficulty: this.mapWebDifficulty(webRec.estimatedTime),
   ...
   */
  private static getUnsafeFunctionExample(
    functionName: string,
  ...
  /**
   * Генерирует массив рекомендаций для веб-безопасности
   */
  ...
  static generateRecommendations(
    input: IssueContext[] | WebSecurityResult
    ...
    return (input as IssueContext[]).map(issue => this.generateRecommendation(issue));
  }
   ...
   */
  static sortByPriority(recommendations: SecurityRecommendation[]): SecurityRecommendation[] {
