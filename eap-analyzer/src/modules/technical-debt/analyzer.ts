...
export class TechnicalDebtAnalyzer extends BaseAnalyzer {
  ...
  private readonly HOURLY_RATE = 50; // долларов в час
  ...
  private getHeatmapColor(score: number): string {
    if (score >= 80) return '#d32f2f'; // красный - критический
    if (score >= 60) return '#f57c00'; // оранжевый - высокий
    if (score >= 40) return '#fbc02d'; // желтый - средний
    if (score >= 20) return '#689f38'; // светло-зеленый - низкий
