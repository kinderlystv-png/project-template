...
export class TechnicalDebtAnalyzer extends BaseAnalyzer {
   ...
   */
  private async analyzeDebtCategories(files: string[]): Promise<any[]> {
    const categories = [
      {
   ...
   */
  private async findDebtHotspots(files: string[]): Promise<any[]> {
    const hotspots = [];
   ...
   */
  private calculateTotalDebt(categories: any[]): number {
   ...
   */
  private calculateMonthlyInterest(totalDebt: number): number {
   ...
   */
  private generatePayoffPlan(categories: any[]): any[] {
    const sortedCategories = [...categories].sort((a, b) => {
      const priorityScore = (cat: any) => {
        const impactWeight: { [key: string]: number } = { Critical: 4, High: 3, Medium: 2, Low: 1 };
   ...
   * ROI анализ
   */
  private calculateROI(categories: any[], totalDebt: number): any {
  ...
  private findComplexMethods(content: string): any[] {
    const methods = [];
    const lines = content.split('\n');
  ...
  private isLargeClass(content: string): boolean {
    const lines = content.split('\n').length;
  ...
  private detectCodeSmells(content: string): number {
    ...
    // Long Parameter List
    if (/\([^)]{80,}\)/.test(content)) smells++;

    // Magic Numbers
    if (/[^a-zA-Z_]\d{2,}[^a-zA-Z_]/.test(content)) smells++;

    // Deep Nesting
    if (/\s{16,}/.test(content)) smells++; // 4+ уровня вложенности
  ...
  private categorizeHotspot(debt: number): string {
  ...
  private calculatePriority(debt: number, issueCount: number): number {
    return Math.min(10, Math.floor(debt / 5) + issueCount);
  }

  private calculateHotspotROI(debt: number): number {
  ...
  private getPriorityLevel(debt: number, impact: string): string {
    const impactWeight: { [key: string]: number } = { Critical: 4, High: 3, Medium: 2, Low: 1 };
  ...
  private calculateCategoryROI(debt: number, refactoringTime: number): number {
    const cost = refactoringTime;
    const yearlyInterest = debt * 0.6;
    return Math.round(((yearlyInterest - cost) / cost) * 100);
  }

  private getROIRecommendation(roi: number, payback: number): string {
  ...
  private calculateROIBreakdown(categories: any[]): any[] {
