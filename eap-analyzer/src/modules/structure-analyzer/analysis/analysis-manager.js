...
export class AnalysisManager {
  ...
  async performFullAnalysis(projectPath, options = {}) {
    ...
    const basicResults = await this.performBasicAnalysis(
      projectPath,
    ...
    if (analysisOptions.enableAdvanced) {
      ...
      advancedResults = await this.performAdvancedAnalysis(
        projectPath,
        thresholds.basic || thresholds
      );
    }
  ...
  async performBasicAnalysis(projectPath, thresholds) {
    try {
      const results = await basicAnalyzer.analyze(projectPath, thresholds);
  ...
  async performAdvancedAnalysis(projectPath, thresholds) {
    try {
      const results = await advancedAnalyzer.analyze(projectPath, thresholds);
  ...
  enrichBasicResults(results) {
    ...
    if (!results.patterns) results.patterns = [];
    if (!results.issues) results.issues = [];
    if (!results.metrics) results.metrics = {};
  ...
  enrichAdvancedResults(results) {
    ...
    // Нормализуем структуру данных
    if (!results.hotspots) results.hotspots = [];
    if (!results.dependencies) results.dependencies = [];
    if (!results.metrics) results.metrics = {};
  ...
  calculateHotspotPriority(hotspot) {
    ...
    // Сложность кода
    if (hotspot.complexity > 20) priority += 30;
    else if (hotspot.complexity > 10) priority += 20;
    else if (hotspot.complexity > 5) priority += 10;

    // Размер файла
    if (hotspot.lines > 1000) priority += 25;
    else if (hotspot.lines > 500) priority += 15;
    else if (hotspot.lines > 200) priority += 5;

    // Количество изменений (если доступно)
    if (hotspot.changeFrequency > 0.8) priority += 20;
    else if (hotspot.changeFrequency > 0.5) priority += 10;
