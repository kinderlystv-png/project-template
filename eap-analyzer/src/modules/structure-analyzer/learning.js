...
class LearningSystem {
  constructor(dataDir = null) {
    ...
    this.metricsHistoryFile = path.join(this.dataDir, 'metrics-history.json');
    this.thresholdsFile = path.join(this.dataDir, 'adaptive-thresholds.json');
    this.patternsFile = path.join(this.dataDir, 'project-patterns.json');
  ...
  loadAdaptiveThresholds() {
    try {
      if (fs.existsSync(this.thresholdsFile)) {
        const data = fs.readFileSync(this.thresholdsFile, 'utf8');
        return JSON.parse(data);
      }
    } catch (error) {
  ...
  saveData() {
    try {
      ...
      fs.writeFileSync(this.thresholdsFile, JSON.stringify(this.adaptiveThresholds, null, 2));
      fs.writeFileSync(this.patternsFile, JSON.stringify(this.projectPatterns, null, 2));
    } catch (error) {
  ...
  calculateAverageComplexity(complexityData) {
    if (!complexityData || complexityData.length === 0) return 0;
  ...
  detectProjectPatterns(basicResults, advancedResults) {
    ...
    if (directories.src) patterns.push('src-structure');
    if (directories.components) patterns.push('component-based');
    if (directories.pages) patterns.push('page-based');
    if (directories.api) patterns.push('api-structure');
    if (directories.tests || directories.test || directories.__tests__)
      patterns.push('well-tested');
    if (directories.docs) patterns.push('documented');

    // Анализ размера проекта
    if (basicResults.totalFiles) {
      if (basicResults.totalFiles < 20) patterns.push('small-project');
      else if (basicResults.totalFiles < 100) patterns.push('medium-project');
      else patterns.push('large-project');
    }

    // Анализ качества кода (если есть расширенные результаты)
    if (advancedResults) {
      if (advancedResults.avgMaintainabilityIndex > 80) patterns.push('high-quality');
      else if (advancedResults.avgMaintainabilityIndex < 50) patterns.push('needs-refactoring');

      if (advancedResults.duplicationPercentage < 3) patterns.push('low-duplication');
      else if (advancedResults.duplicationPercentage > 10) patterns.push('high-duplication');

      if (advancedResults.hotspotCount === 0) patterns.push('no-hotspots');
      else if (advancedResults.hotspotCount > 10) patterns.push('many-hotspots');
    }
  ...
  adaptThresholds() {
    try {
      ...
      if (!aggregated.basic || aggregated.count < 5) return;
      ...
      if (aggregated.advanced && aggregated.advanced.avgComplexity) {
        const newMaxComplexity = Math.max(
          10,
          Math.round(aggregated.advanced.avgComplexity.avg * 2)
        );
    ...
    for (const project of projects) {
      ...
      if (commonPatterns.length >= 2) {
        similarProjects.push({
          ...project,
          similarity:
            commonPatterns.length / Math.max(currentPatterns.length, project.patterns.length),
        });
      }
    }
  ...
  assessDataQuality() {
    ...
    if (projectCount < 5) return 'insufficient';
    if (projectCount < 20) return 'moderate';
    if (projectCount < 50) return 'good';
    return 'excellent';
  }
  ...
  cleanOldData(maxAgeInDays = 365) {
    try {
      ...
      this.metricsHistory.projects = this.metricsHistory.projects.filter(
        project => new Date(project.timestamp) > cutoffDate
      );
