  ...
  async analyzeProjectStructure(projectPath, options = {}) {
    ...
    try {
      ...
      if (analysisOptions.enableAdvanced) {
        ...
        advancedResults = await advancedAnalyzer.analyze(
          projectPath,
          thresholds.basic || thresholds
        );
      }
      ...
      // 6. Интеграция с ЭАП
      const eapIntegration = this.generateEAPIntegration(
        basicResults,
        advancedResults,
        recommendations
      );
  ...
  async validateProjectSize(projectPath) {
    try {
      ...
      if (fileCount > this.config.performanceSettings.maxFiles) {
        console.warn(
          `[StructureAnalyzer] Проект содержит ${fileCount} файлов (лимит: ${this.config.performanceSettings.maxFiles})`
        );
      }
    } catch (error) {
...
`;
  ...
  buildRefactoringRoadmap(results) {
    ...
    if (results.advanced && results.advanced.hotspots && results.advanced.hotspots.length > 0) {
      const complexityHotspots = results.advanced.hotspots.filter(h =>
        h.reasons.some(r => r.includes('сложност'))
      );
      const sizeHotspots = results.advanced.hotspots.filter(h =>
        h.reasons.some(r => r.includes('размер'))
      );
      const duplicationHotspots = results.advanced.hotspots.filter(h =>
        h.reasons.some(r => r.includes('дублирован'))
      );
      const maintainabilityHotspots = results.advanced.hotspots.filter(h =>
        h.reasons.some(r => r.includes('сопровожда'))
      );
    ...
    if (results.basic?.filesToRefactor > 0) {
      ...
      if (problematicDirs.length > 0) {
        problematicDirs.forEach(([dir, stats]) => {
          roadmapItems.push({
            title: `Реорганизация модуля "${path.basename(dir)}"`,
  ...
  estimateTestCoverage(basicResults) {
    if (basicResults.testFiles === 0) return 0;

    const testRatio =
      basicResults.testFiles / Math.max(1, basicResults.totalFiles - basicResults.testFiles);
  ...
  getQuickRecommendation(score) {
    if (score >= 90) return 'Отличная структура проекта';
    if (score >= 80) return 'Хорошая структура, есть место для улучшений';
    if (score >= 70) return 'Удовлетворительная структура, рекомендуется рефакторинг';
    if (score >= 60) return 'Структура требует внимания';
  ...
  generateSummary(basicResults, advancedResults, recommendations) {
    ...
    if (
      ...
      summary.issues.push(
        `${advancedResults.circularDependencies.length} циклических зависимостей`
      );
    }
  ...
  getRefactoringNecessity(score, criticalIssues) {
    if (score < 60 || criticalIssues > 5) return 'ВЫСОКАЯ';
    if (score < 80 || criticalIssues > 2) return 'СРЕДНЯЯ';
    return 'НИЗКАЯ';
  }
  ...
  async generateFullReport(projectPath, options = {}) {
    ...
    const roadmapContent = await this.generateRefactoringRoadmap(
      results.basic,
      results.advanced,
    ...
    return {
      ...
      roadmap: {
        ...
        metadata: {
          ...
          projectPath,
          analysisVersion: config.module.version,
  ...
  calculateROI(summary) {
    ...
    return {
      ...
      paybackPeriod:
        refactoringCost > 0 ? Math.round((refactoringCost / (annualBenefit / 12)) * 10) / 10 : 0,
  ...
  getROIRecommendation(benefit, cost) {
    ...
    if (roi > 200) return 'Крайне выгодный рефакторинг - немедленно приступать';
    if (roi > 100) return 'Высоко прибыльный рефакторинг - рекомендуется';
    if (roi > 50) return 'Прибыльный рефакторинг - стоит рассмотреть';
    if (roi > 0) return 'Умеренно прибыльный - оценить приоритеты';
...
export { analyzeProjectStructure, quickStructureCheck, getModuleInfo, exportResults };
