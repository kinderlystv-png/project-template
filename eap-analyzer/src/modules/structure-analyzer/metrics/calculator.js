...
export class MetricsCalculator {
  ...
  calculateBasicScore(basicResults) {
    ...
    if (basicResults.avgFileSize > this.core.config.thresholds.maxFileSize) {
      score -= Math.min(
        30,
        (basicResults.avgFileSize - this.core.config.thresholds.maxFileSize) / 10
      );
    }
  ...
  scoreFileSize(avgSize) {
    if (!avgSize) return 100;
    if (avgSize < 200) return 100;
    if (avgSize < 500) return 80;
    if (avgSize < 1000) return 60;
    if (avgSize < 2000) return 40;
    return 20;
  }
  ...
  scoreDuplication(duplicationPercentage) {
    if (!duplicationPercentage) return 100;
    if (duplicationPercentage < 2) return 100;
    if (duplicationPercentage < 5) return 80;
    if (duplicationPercentage < 10) return 60;
    if (duplicationPercentage < 20) return 40;
    return 20;
  }
  ...
  scoreTestCoverage(coverage) {
    ...
    if (coverage >= 60) return 80;
    if (coverage >= 40) return 60;
    if (coverage >= 20) return 40;
  ...
  generateMetricsSummary(basicResults, advancedResults, recommendations) {
    ...
    const maintainabilityMetrics = this.calculateMaintainabilityMetrics(
      basicResults,
      advancedResults
    );
