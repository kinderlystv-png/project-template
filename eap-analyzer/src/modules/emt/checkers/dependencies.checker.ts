...
export class EMTDependenciesChecker extends BaseChecker {
  readonly name = 'EMT Dependencies Checker';
  readonly category = 'security' as const;
  readonly description = 'Проверяет состояние и безопасность зависимостей EMT проекта';
    ...
    try {
      ...
      if (!hasSecurityScript) {
        recommendations.push(
          'Добавьте npm scripts для проверки безопасности: "audit": "npm audit"'
        );
      }
      ...
      return this.createResult(
        passed,
        score,
        `Dependencies check ${passed ? 'passed' : 'failed'} (${score}/100)`,
        {
          issues,
          warnings,
          totalDependencies: Object.keys(dependencies).length,
          emtPackages: Object.keys(dependencies).filter(
            pkg => pkg.includes('emt') || pkg.includes('@emt/')
          ),
        },
        recommendations
      );
