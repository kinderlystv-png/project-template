...
export class EMTConfigChecker extends BaseChecker {
  readonly name = 'EMT Configuration Checker';
  readonly category = 'quality' as const;
  readonly description = 'Проверяет корректность конфигурации EMT проекта';
  ...
  async check(context: CheckContext): Promise<CheckResult> {
    ...
    try {
      ...
      return this.createResult(
        passed,
        score,
        `Configuration check ${passed ? 'passed' : 'failed'} (${score}/100)`,
        { issues, warnings },
        recommendations
      );
    } catch (error) {
