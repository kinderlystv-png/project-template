...
import { PatternMatchers } from './pattern-matchers';
import {
  ALL_PATTERNS,
  PatternDefinition,
  AnalysisContext,
...
export interface CodePattern {
  id: string;
  name: string;
  type: 'architectural' | 'design' | 'antipattern' | 'performance' | 'security';
  ...
  examples: string[];
  // Новые поля для улучшенной диагностики
  evidence?: string[];
  matchDetails?: {
    matchedConditions: number;
    totalConditions: number;
    criticalMatches: string[];
    confidenceModifiers: Array<{
      description: string;
      impact: number;
    }>;
  };
...
export class PatternRecognizer {
  private readonly CONFIDENCE_THRESHOLD = 50; // Снижен для лучшего покрытия
   ...
   */
  async analyzePatterns(files: FileStructure[]): Promise<PatternAnalysisResult> {
    ...
    detectedPatterns.push(
      ...(await this.analyzePatternGroup('architectural', ARCHITECTURAL_PATTERNS))
    );
    detectedPatterns.push(...(await this.analyzePatternGroup('design', DESIGN_PATTERNS)));
    detectedPatterns.push(...(await this.analyzePatternGroup('antipattern', ANTIPATTERNS)));
    detectedPatterns.push(...(await this.analyzePatternGroup('security', SECURITY_PATTERNS)));

    // Фильтруем по уровню уверенности
    const significantPatterns = detectedPatterns.filter(
    ...
    console.log(
      `✅ Обнаружено ${significantPatterns.length} значимых паттернов (улучшенный алгоритм)`
    );
   ...
   */
  private updateContext(files: FileStructure[]): void {
   ...
   */
  private analyzePatternDefinition(definition: PatternDefinition): CodePattern | null {
    ...
    for (const matcher of definition.matchers) {
      const score = PatternMatchers.matchCombined(this.context, [matcher]);
    ...
    const appliedModifiers: Array<{ description: string; impact: number }> = [];
    ...
    const pattern: CodePattern = {
      ...
      type: definition.category as any,
   ...
   */
  private generateEvidence(matcher: any, score: number): string {
   ...
   */
  private generateLocations(evidence: string[]): PatternLocation[] {
   ...
   */
  private generateRecommendation(definition: PatternDefinition): string {
   ...
   */
  private generateExamples(definition: PatternDefinition): string[] {
   ...
   */
  private async detectArchitecturalPatterns(files: FileStructure[]): Promise<CodePattern[]> {
    // Используем новый улучшенный алгоритм
    this.updateContext(files);
    return this.analyzePatternGroup('architectural', ARCHITECTURAL_PATTERNS);
  }

  /**
   * Обнаруживает паттерны проектирования
   */
  private async detectDesignPatterns(files: FileStructure[]): Promise<CodePattern[]> {
    return this.analyzePatternGroup('design', DESIGN_PATTERNS);
  }

  /**
   * Обнаруживает антипаттерны
   */
  private async detectAntipatterns(files: FileStructure[]): Promise<CodePattern[]> {
    return this.analyzePatternGroup('antipattern', ANTIPATTERNS);
  }

  /**
   * Обнаруживает паттерны производительности
   */
  private async detectPerformancePatterns(files: FileStructure[]): Promise<CodePattern[]> {
    const patterns: CodePattern[] = [];

    // N+1 Query Problem (улучшенное обнаружение)
    const nPlusOnePattern = this.detectNPlusOneQueries(files);
    if (nPlusOnePattern) patterns.push(nPlusOnePattern);

    // Memory Leaks (улучшенное обнаружение)
    const memoryLeakPattern = this.detectMemoryLeaks(files);
    if (memoryLeakPattern) patterns.push(memoryLeakPattern);

    // Inefficient Loops (улучшенное обнаружение)
    const inefficientLoopsPattern = this.detectInefficientLoops(files);
    if (inefficientLoopsPattern) patterns.push(inefficientLoopsPattern);
   ...
   */
  private async detectSecurityPatterns(files: FileStructure[]): Promise<CodePattern[]> {
    return this.analyzePatternGroup('security', SECURITY_PATTERNS);
  }

  /**
   * Обнаруживает паттерн MVC
   */
  private detectMVCPattern(files: FileStructure[]): CodePattern | null {
    ...
    const confidence = Math.min(
      100,
      controllers.length * 20 + models.length * 25 + views.length * 15
    );
    ...
    return {
      ...
      confidence,
      description:
        'Архитектурный паттерн MVC разделяет логику на модели, представления и контроллеры',
      impact: 'medium',
      locations: [
      ...
      recommendation:
        'Хорошая архитектурная основа. Убедитесь в четком разделении ответственности между слоями.',
      examples: [
   ...
   */
  private detectMicroservicesPattern(files: FileStructure[]): CodePattern | null {
    const serviceIndicators = files.filter(
      f =>
    ...
    const apiGateways = files.filter(
      f =>
        f.content.includes('proxy') || f.content.includes('gateway') || f.content.includes('nginx')
    );

    const confidence = Math.min(
      100,
      serviceIndicators.length * 15 + dockerFiles.length * 25 + apiGateways.length * 30
    );
    ...
    return {
      ...
      recommendation:
        'Отличная масштабируемая архитектура. Обратите внимание на мониторинг и обработку ошибок между сервисами.',
      examples: [
   ...
   */
  private detectLayeredArchitecture(files: FileStructure[]): CodePattern | null {
    const layers = {
      presentation: files.filter(
        f =>
          f.path.includes('ui/') || f.path.includes('frontend/') || f.path.includes('components/')
      ),
      business: files.filter(
        f =>
          f.path.includes('business/') || f.path.includes('logic/') || f.path.includes('services/')
      ),
      data: files.filter(
        f => f.path.includes('data/') || f.path.includes('dao/') || f.path.includes('repository/')
      ),
    };
    ...
    return {
      ...
      recommendation:
        'Хорошая организация кода по слоям. Избегайте нарушения зависимостей между слоями.',
      examples: [
   ...
   */
  private detectComponentPattern(files: FileStructure[]): CodePattern | null {
    ...
    if (componentFiles.length < 3) return null;
    ...
    return {
      ...
      recommendation:
        'Отличная модульность. Следите за размером компонентов и избегайте глубокой вложенности.',
      examples: [
   ...
   */
  private createPatternLocation(file: FileStructure, context: string): PatternLocation {
   ...
   */
  private analyzeResults(patterns: CodePattern[]): Omit<PatternAnalysisResult, 'detectedPatterns'> {
    ...
    const architecturalScore = Math.max(
      0,
      100 -
    ...
    const predominantArchitecture =
      architecturalPatterns.length > 0
        ? architecturalPatterns.reduce((prev, current) =>
    ...
    return {
      ...
      summary: {
        ...
        problematicPatterns:
          antipatterns.length + securityConcerns.length + performanceIssues.length,
  ...
  private generateSummaryRecommendations(
    ...
    if (criticalPatterns.length > 0) {
      recommendations.push(
        `🚨 КРИТИЧНО: Обнаружено ${criticalPatterns.length} критических проблем требующих немедленного исправления!`
      );
    }
    ...
    if (architecturalScore < 50) {
      recommendations.push(
        '🏗️ Архитектура требует серьезной переработки. Рассмотрите рефакторинг.'
      );
    } else if (architecturalScore < 70) {
      recommendations.push(
        '🔧 Архитектура нуждается в улучшениях. Устраните выявленные антипаттерны.'
      );
    } else {
      recommendations.push(
        '✅ Архитектура в хорошем состоянии. Продолжайте следовать лучшим практикам.'
      );
    }
    ...
    if (designQuality < 60) {
      recommendations.push(
        '📐 Качество дизайна низкое. Применяйте принципы SOLID и паттерны проектирования.'
      );
    }
    ...
    if (securityIssues > 0) {
      recommendations.push(
        `🔒 Обнаружено ${securityIssues} проблем безопасности. Проведите security аудит.`
      );
    }
    ...
    if (performanceIssues > 0) {
      recommendations.push(
        `⚡ Обнаружено ${performanceIssues} проблем производительности. Оптимизируйте алгоритмы.`
      );
    }
   ...
   */
  private detectNPlusOneQueries(files: FileStructure[]): CodePattern | null {
    ...
    return {
      ...
      recommendation:
        'КРИТИЧНО для производительности: Используйте eager loading, batch queries или JOIN запросы вместо запросов в циклах.',
      examples: [
   ...
   */
  private detectMemoryLeaks(files: FileStructure[]): CodePattern | null {
    const suspiciousFiles = files.filter(f => {
      const content = f.content.toLowerCase();
    ...
    return {
      ...
      recommendation:
        'Убедитесь в правильной очистке event listeners, timers и других ресурсов. Используйте cleanup методы.',
      examples: [
   ...
   */
  private detectInefficientLoops(files: FileStructure[]): CodePattern | null {
    const suspiciousFiles = files.filter(f => {
      const content = f.content;
      const nestedLoops = (content.match(/for.*{[\s\S]*?for/g) || []).length;
      const inefficientOperations =
        (content.includes('indexOf') && content.includes('for')) ||
    ...
    return {
      ...
      recommendation:
        'Рассмотрите использование более эффективных алгоритмов: Map/Set для поиска, уменьшение вложенности циклов.',
      examples: [
