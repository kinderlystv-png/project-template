...
export class DockerAnalyzer extends BaseAnalyzer {
  ...
  async analyze(projectPath: string): Promise<AnalysisResult> {
    ...
    try {
      ...
      if (!metrics.hasNonRootUser && metrics.hasDockerfile) {
        warnings.push(
          'Docker контейнер запускается от root пользователя - это угроза безопасности'
        );
      }
      ...
      return this.createResult(
        false,
        null,
        [`Ошибка анализа Docker: ${error instanceof Error ? error.message : 'Неизвестная ошибка'}`],
        [],
        filesAnalyzed
      );
    }
  ...
  private analyzeDockerfile(content: string): {
    hasMultiStage: boolean;
    hasNonRootUser: boolean;
    hasHealthCheck: boolean;
    securityScore: number;
    ...
    const hasNonRootUser = userLines.some(
      line => !line.includes('USER root') && !line.includes('USER 0')
    );
    ...
    if (
      lines.some(line => line.includes('RUN apt-get update') && !line.includes('apt-get clean'))
    ) {
  ...
  private analyzeDockerCompose(content: string): {
    issues: string[];
  } {
    const issues: string[] = [];
  ...
  private analyzeDockerignore(content: string): {
    issues: string[];
  } {
    const issues: string[] = [];
    const lines = content
