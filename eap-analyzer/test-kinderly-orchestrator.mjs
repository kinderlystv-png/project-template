 ...
 * Использует ES-модули для корректной работы
 */
...
async function runFullAnalysis() {
  try {
    ...
    if (result.fileIssues && Object.keys(result.fileIssues).length > 0) {
      ...
      if (Object.keys(result.fileIssues).length > 5) {
        console.log(`\n... и еще ${Object.keys(result.fileIssues).length - 5} файлов с проблемами`);
      }
    }
