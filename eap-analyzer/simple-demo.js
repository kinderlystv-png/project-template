...
class SimpleEAPDemo {
  ...
  showResults(stats, aiAnalysis, debtAnalysis) {
    ...
    console.log(
      `   🔄 Дублирование: ${aiAnalysis.duplication.percentage.toFixed(1)}% (${aiAnalysis.duplication.blocks} блоков)`
    );
    ...
    debtAnalysis.hotspots.forEach(hotspot => {
      console.log(
        `      • ${hotspot.file}: ${hotspot.debt} часов (${hotspot.issues.length} проблем)`
      );
    });
    console.log('');
