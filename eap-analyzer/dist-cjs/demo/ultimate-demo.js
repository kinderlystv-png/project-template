...
exports.EAPAnalyzerDemo = void 0;
const orchestrator_js_1 = require("../core/orchestrator.js");
const ultimate_report_generator_js_1 = require("../core/ultimate-report-generator.js");
...
class EAPAnalyzerDemo {
    ...
    ultimateGenerator;
    constructor() {
        this.orchestrator = new orchestrator_js_1.AnalysisOrchestrator(6); // 6 параллельных потоков
        this.ultimateGenerator = new ultimate_report_generator_js_1.UltimateReportGenerator();
    }
    async runFullDemo() {
    ...
    async demonstrateResults(analysis, report) {
    ...
    showRecommendations(report) {
        ...
        if (report.ultimateAnalysis?.aiInsights?.aiRecommendations?.length > 0) {
            ...
            report.ultimateAnalysis.aiInsights.aiRecommendations.slice(0, 3).forEach((rec) => {
                console.log(`      • ${rec}`);
            });
        }
        if (report.refactoringRecommendations?.opportunities?.length > 0) {
            console.log('   🔧 Возможности рефакторинга:');
            report.refactoringRecommendations.opportunities.slice(0, 3).forEach((opp) => {
        ...
        if (report.technicalDebtAnalysis?.payoffPlan?.length > 0) {
            ...
            report.technicalDebtAnalysis.payoffPlan.slice(0, 3).forEach((plan) => {
                console.log(`      • ${plan.category}: ${plan.refactoringTime} часов (${plan.priority})`);
            });
        }
        console.log('');
    }
    async saveReports(analysis, report) {
        ...
        try {
            ...
            if (report.technicalDebtAnalysis) {
                ...
                await fs.promises.writeFile(debtPath, JSON.stringify(report.technicalDebtAnalysis, null, 2));
    ...
    async generateMarkdownSummary(report, dir) {
        ...
        if (report.aiInsights) {
            ...
            if (report.aiInsights.patterns?.length > 0) {
                ...
                report.aiInsights.patterns.forEach((pattern) => {
        ...
        if (report.ultimateAnalysis?.aiInsights?.aiRecommendations?.length > 0) {
            report.ultimateAnalysis.aiInsights.aiRecommendations.slice(0, 5).forEach((rec) => {
                md += `- ${rec}\n`;
            });
        }
...
exports.EAPAnalyzerDemo = EAPAnalyzerDemo;
// Запуск демонстрации
try {
...
//# sourceMappingURL=ultimate-demo.js.map