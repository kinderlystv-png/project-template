...
exports.UltraEnhancedAnalyzer = void 0;
const architecture_analyzer_1 = require("./modules/architecture-analyzer");
const technical_debt_1 = require("./modules/technical-debt");
const refactoring_analyzer_1 = require("./modules/refactoring-analyzer");
class UltraEnhancedAnalyzer {
    architectureAnalyzer;
    technicalDebtAnalyzer;
    refactoringAnalyzer;
    constructor() {
        this.architectureAnalyzer = new architecture_analyzer_1.ArchitectureAnalyzer();
        this.technicalDebtAnalyzer = new technical_debt_1.TechnicalDebtAnalyzer();
        this.refactoringAnalyzer = new refactoring_analyzer_1.RefactoringAnalyzer();
    }
    ...
    generateSummary(architecture, technicalDebt, refactoring) {
        ...
        // Recommend strategy
        const recommendedStrategy = this.recommendStrategy(overallScore, riskLevel, technicalDebt, refactoring);
    ...
    generateInsights(architecture, technicalDebt, refactoring) {
        const strengths = [];
        const weaknesses = [];
        const opportunities = [];
        const threats = [];
        ...
        if (architecture.patterns.length > 0) {
            strengths.push(`Well-structured architecture with ${architecture.patterns.length} recognized patterns`);
        }
        ...
        if (technicalDebt.categories.length > 0) {
            ...
            if (criticalDebt.length > 0) {
                threats.push(`${criticalDebt.length} critical technical debt items threatening system stability`);
            }
        }
    ...
    generateRoadmap(architecture, technicalDebt, refactoring) {
        const immediate = [];
        const shortTerm = [];
        const longTerm = [];
        ...
        if (architecture.scalability.recommendations.length > 0) {
            architecture.scalability.recommendations.slice(0, 2).forEach((rec, index) => {
                shortTerm.push({
    ...
    calculateArchitectureScore(architecture) {
        ...
        const modularityScore = architecture.modularity.cohesion * 50 + (1 - architecture.modularity.coupling) * 50;
    ...
    assessRiskLevel(architecture, technicalDebt, refactoring) {
        ...
        if (technicalDebt.totalDebt.totalDays > 15 ||
    ...
    generatePriorityActions(architecture, technicalDebt, refactoring) {
        const actions = [];
        const criticalDebt = technicalDebt.categories.filter(c => c.priority === 'critical');
        if (criticalDebt.length > 0) {
    ...
    recommendStrategy(overallScore, riskLevel, technicalDebt, refactoring) {
    ...
    calculateEstimatedEffort(technicalDebt, refactoring) {
...
exports.UltraEnhancedAnalyzer = UltraEnhancedAnalyzer;
//# sourceMappingURL=ultra-enhanced-analyzer.js.map