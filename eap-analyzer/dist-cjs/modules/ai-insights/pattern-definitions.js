...
exports.ALL_PATTERNS = exports.SECURITY_PATTERNS = exports.ANTIPATTERNS = exports.DESIGN_PATTERNS = exports.ARCHITECTURAL_PATTERNS = void 0;
// Улучшенные определения архитектурных паттернов
exports.ARCHITECTURAL_PATTERNS = [
    {
        id: 'mvc',
        ...
        confidenceModifiers: [
            {
                condition: ctx => ctx.directories.some(d => /^(src\/)?(models?|views?|controllers?)$/i.test(d)),
                modifier: 1.3,
        ...
        confidenceModifiers: [
            {
                condition: ctx => {
                    let factoryCount = 0;
                    for (const [_, content] of ctx.fileContents) {
                        if (/class\s+\w+Factory/i.test(content))
                            factoryCount++;
                    }
        ...
        confidenceModifiers: [
            {
                condition: ctx => {
                    let strategyCount = 0;
                    for (const [_, content] of ctx.fileContents) {
                        if (/class\s+\w+Strategy/i.test(content))
                            strategyCount++;
                    }
        ...
        confidenceModifiers: [
            {
                condition: ctx => {
                    let magicCount = 0;
                    for (const [_, content] of ctx.fileContents) {
                        const matches = content.match(/(?<![a-zA-Z_$])\b(?!0[xX])\d{2,}\b(?![a-zA-Z_$])/g) || [];
...
//# sourceMappingURL=pattern-definitions.js.map