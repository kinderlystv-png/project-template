...
exports.QualityPredictor = void 0;
class QualityPredictor {
    ...
    predict(features) {
        ...
        console.log(`✅ Качество предсказано: ${qualityScore.overall}/100 (уверенность: ${qualityScore.confidence}%)`);
        return qualityScore;
    }
    /**
     * Рассчитывает поддерживаемость кода
     */
    calculateMaintainability(features) {
        let score = 100;
    ...
    calculateOverallScore(scores) {
        ...
        const totalWeight = weights.maintainability + weights.reliability + securityWeight + weights.performance;
        const weightedScore = (scores.maintainability * weights.maintainability +
    ...
    calculateConfidence(features) {
        ...
        const totalPatterns = features.goodPatternCount +
     ...
     */
    generateRecommendations(features, qualityScore) {
        ...
        if (features.securityIssueCount > 0) {
            recommendations.push({
                ...
                description: 'Обнаружены серьезные уязвимости безопасности, требующие немедленного исправления',
                impact: 'high',
                effort: 'medium',
                priority: 10,
                actions: [
     ...
     */
    createPredictionSummary(qualityScore, features) {
        const { thresholds } = this.rules;
        let qualityLevel = 'критическое';
        if (qualityScore.overall >= thresholds.excellent)
            qualityLevel = 'отличное';
        else if (qualityScore.overall >= thresholds.good)
            qualityLevel = 'хорошее';
        else if (qualityScore.overall >= thresholds.fair)
            qualityLevel = 'удовлетворительное';
        else if (qualityScore.overall >= thresholds.poor)
            qualityLevel = 'плохое';
        const summary = [
...
exports.QualityPredictor = QualityPredictor;
//# sourceMappingURL=quality-predictor.js.map