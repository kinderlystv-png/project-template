...
exports.PatternMatchers = void 0;
const path = __importStar(require("path"));
class PatternMatchers {
    /**
     * Улучшенный матчер для файлов
     */
    static matchFile(filePath, patterns) {
     ...
     */
    static matchDirectory(directories, patterns) {
        let matchScore = 0;
        let matchCount = 0;
        for (const dir of directories) {
            ...
            for (const pattern of patterns) {
                ...
                if (matched)
                    break; // Избегаем двойного подсчета одной директории
            }
        }
     ...
     */
    static matchContent(content, patterns) {
     ...
     * Матчер для импортов
     */
    static matchImports(imports, patterns) {
        if (imports.length === 0)
            return 0;
        let matchScore = 0;
        ...
        for (const imp of imports) {
            for (const pattern of patterns) {
                ...
                if (matched)
                    break; // Считаем каждый импорт только один раз
            }
        }
        if (matchedImports === 0)
            return 0;
     ...
     * Матчер для классов
     */
    static matchClasses(classes, patterns) {
        if (classes.size === 0)
            return 0;
        let matchScore = 0;
     ...
     * Матчер для функций
     */
    static matchFunctions(functions, patterns) {
        if (functions.size === 0)
            return 0;
        let matchScore = 0;
     ...
     */
    static matchCombined(context, matchers) {
        let totalScore = 0;
        let totalWeight = 0;
            ...
            try {
                switch (matcher.type) {
                    case 'file':
                        ...
                        break;
                    case 'directory':
                        score = this.matchDirectory(context.directories || [], matcher.patterns || [matcher.pattern]);
                        break;
                    case 'content':
                        if (context.fileContents) {
                            for (const [_, content] of context.fileContents) {
                                const contentScore = this.matchContent(content, matcher.patterns || [matcher.pattern]);
                    ...
                    case 'import':
                        ...
                        break;
                    case 'class':
                        score = this.matchClasses(context.classes || new Map(), matcher.patterns || [matcher.pattern]);
                        break;
                    case 'function':
                        score = this.matchFunctions(context.functions || new Map(), matcher.patterns || [matcher.pattern]);
                        break;
                }
            }
            catch (error) {
     ...
     */
    static hasCodePattern(content, pattern) {
        try {
            if (typeof pattern === 'string') {
     ...
     */
    static countPatternOccurrences(content, pattern) {
        try {
            if (typeof pattern === 'string') {
     ...
     */
    static analyzeMatchQuality(matches, totalPatterns, contentSize) {
...
exports.PatternMatchers = PatternMatchers;
//# sourceMappingURL=pattern-matchers.js.map