...
exports.DockerOptimizationChecker = void 0;
const checker_js_1 = require("../../../core/checker.js");
class DockerOptimizationChecker extends checker_js_1.BaseChecker {
    name = 'Docker Optimization Checker';
    category = 'performance';
    description = 'Проверяет оптимизацию Docker образов и производительность';
    async check(context) {
        ...
        try {
            ...
            if (fs.existsSync(dockerfilePath)) {
                ...
                if (!hasMultiStage && lines.length > 20) {
                    ...
                    score -= 15;
                    recommendations.push('Рассмотрите использование multi-stage build для уменьшения размера образа');
                }
                ...
                if (runLines.length > 5) {
                    ...
                    score -= 10;
                    recommendations.push('Объедините RUN команды с помощью && для уменьшения количества слоев');
                }
                ...
                for (const { pattern, manager } of packageManagerPatterns) {
                    const hasPattern = lines.some((line) => pattern.test(line));
                    if (hasPattern) {
                        // Проверка очистки кэша
                        if (manager === 'apt') {
                            const hasCleanup = lines.some((line) => line.includes('apt-get clean') || line.includes('rm -rf /var/lib/apt/lists/*'));
                            if (!hasCleanup) {
                                warnings.push('apt-get без очистки кэша увеличивает размер образа');
                                score -= 10;
                                recommendations.push('Добавьте очистку кэша: && apt-get clean && rm -rf /var/lib/apt/lists/*');
                            }
                        }
                        if (manager === 'npm') {
                            const hasCleanup = lines.some((line) => line.includes('npm cache clean'));
                            if (!hasCleanup) {
                ...
                const addLines = lines.filter((line) => line.startsWith('ADD'));
                const unnecessaryAdd = addLines.filter((line) => !line.includes('http') && !line.includes('.tar'));
                if (unnecessaryAdd.length > 0) {
                    warnings.push('Используйте COPY вместо ADD для простых файлов');
                    score -= 5;
                    recommendations.push('ADD предназначен для архивов и URL, используйте COPY для обычных файлов');
                }
                // Проверка базового образа
                const baseImages = fromLines.map((line) => line.split(' ')[1]);
                for (const image of baseImages) {
                    ...
                    if (image &&
                        ...
                        !image.includes('slim')) {
                        warnings.push(`Используется полный образ ${image} - рассмотрите alpine или slim варианты`);
                        score -= 10;
                ...
                if (!fs.existsSync(dockerignorePath)) {
                    ...
                    score -= 15;
                    recommendations.push('Создайте .dockerignore для исключения ненужных файлов из контекста сборки');
                }
                else {
                    ...
                    const missingIgnores = importantIgnores.filter(ignore => !ignoreLines.some((line) => line.includes(ignore)));
                    if (missingIgnores.length > 0) {
                        warnings.push(`В .dockerignore отсутствуют важные исключения: ${missingIgnores.join(', ')}`);
                ...
                // Проверка LABEL
                const hasLabels = lines.some((line) => line.startsWith('LABEL'));
                if (!hasLabels) {
                ...
                return this.createResult(true, 100, 'No Dockerfile found - optimization check not applicable', { reason: 'No Docker configuration' });
            }
            ...
            for (const composeFile of composeFiles) {
                ...
                if (fs.existsSync(composePath)) {
                    ...
                    if (!composeContent.includes('restart:')) {
                        ...
                        score -= 5;
                        recommendations.push('Добавьте restart: unless-stopped для автоматического перезапуска');
                    }
                    // Проверка networks
            ...
            return this.createResult(passed, score, `Docker optimization check ${passed ? 'passed' : 'failed'} (${score}/100)`, { issues, warnings }, recommendations);
        }
        catch (error) {
...
exports.DockerOptimizationChecker = DockerOptimizationChecker;
//# sourceMappingURL=optimization.checker.js.map