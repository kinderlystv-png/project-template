...
exports.DockerSecurityChecker = void 0;
const checker_js_1 = require("../../../core/checker.js");
class DockerSecurityChecker extends checker_js_1.BaseChecker {
    name = 'Docker Security Checker';
    category = 'security';
    description = 'Проверяет безопасность Docker конфигураций и контейнеров';
    async check(context) {
        ...
        try {
            ...
            if (fs.existsSync(dockerfilePath)) {
                ...
                const hasNonRootUser = userLines.some((line) => !line.includes('USER root') && !line.includes('USER 0') && line.includes('USER'));
                if (!hasNonRootUser) {
                    issues.push('Контейнер запускается от root пользователя');
                    score -= 30;
                    recommendations.push('Создайте non-root пользователя: RUN adduser --disabled-password --gecos "" appuser && USER appuser');
                }
                ...
                for (let i = 0; i < lines.length; i++) {
                    ...
                    for (const pattern of sensitivePatterns) {
                        if (pattern.test(line)) {
                            ...
                            score -= 40;
                            recommendations.push('Используйте Docker secrets или переменные окружения для чувствительных данных');
                            break;
                        }
                    }
                }
                ...
                if (!lines.some((line) => line.startsWith('HEALTHCHECK'))) {
            ...
            for (const composeFile of composeFiles) {
                ...
                if (fs.existsSync(composePath)) {
                    ...
                    if (composeContent.includes('privileged: true')) {
                        issues.push('Обнаружен privileged режим в docker-compose - критическая угроза безопасности');
                        score -= 35;
                    ...
                    if (composeContent.includes(':/')) {
                        const volumeLines = composeContent
                            .split('\n')
                            .filter((line) => line.includes(':/') && line.includes('volumes:'));
                    ...
                    if (envLines.length > 0) {
                        ...
                        score -= 10;
                        recommendations.push('Используйте Docker secrets вместо environment переменных для чувствительных данных');
                    }
                }
            }
            // Финальная оценка
            ...
            return this.createResult(passed, score, `Docker security check ${passed ? 'passed' : 'failed'} (${score}/100)`, { issues, warnings }, recommendations);
        }
        catch (error) {
...
exports.DockerSecurityChecker = DockerSecurityChecker;
