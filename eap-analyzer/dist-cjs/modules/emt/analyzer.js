...
class EMTAnalyzer extends analyzer_js_1.BaseAnalyzer {
    metadata = {
    ...
    async analyze(projectPath) {
        ...
        try {
            ...
            return this.createResult(false, null, [`Ошибка анализа EMT: ${error instanceof Error ? error.message : 'Неизвестная ошибка'}`], [], filesAnalyzed);
        }
    }
    ...
    async detectEMTFramework(projectPath) {
        const path = require('path');
        const fs = require('fs');
        const packagePath = path.join(projectPath, 'package.json');
        if (!fs.existsSync(packagePath)) {
    ...
    async analyzeServices(projectPath) {
        // Упрощенная реализация - в реальности нужен парсер кода
        return [];
    }
}
