...
async function analyze(projectRoot, config) {
    ...
    const files = await basic_analyzer_js_1.default.collectFiles(projectRoot, config);
    const results = {
    ...
    for (const file of files) {
        try {
            const fullPath = path_1.default.join(projectRoot, file);
...
function analyzeFileComplexity(filePath, content, config) {
    ...
    const maintainabilityIndex = calculateMaintainabilityIndex(linesOfCode, cyclomaticComplexity, content);
    // Глубина вложенности
    ...
    const needsRefactoring = linesOfCode > config.thresholds.maxFileSize ||
    ...
    return {
        ...
        functionCount,
        needsRefactoring,
        refactoringReasons: getRefactoringReasons(linesOfCode, cyclomaticComplexity, maintainabilityIndex, maxNestingDepth, config),
    };
}
/**
 ...
 */
function getRefactoringReasons(linesOfCode, cyclomaticComplexity, maintainabilityIndex, maxNestingDepth, config) {
    const reasons = [];
...
function calculateModuleCohesion(files) {
    ...
    for (let i = 0; i < files.length; i++) {
        for (let j = i + 1; j < files.length; j++) {
            const file1 = path_1.default.basename(files[i].path, path_1.default.extname(files[i].path));
            const file2 = path_1.default.basename(files[j].path, path_1.default.extname(files[j].path));
