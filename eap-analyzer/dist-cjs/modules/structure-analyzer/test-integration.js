...
exports.testModularIntegration = testModularIntegration;
...
async function testModularIntegration() {
    ...
    try {
        ...
        console.log(`⚙️ Модули загружены: ${Object.keys(analyzer).filter(k => k.includes('Manager') || k.includes('Calculator') || k.includes('Generator') || k.includes('Integration')).length}\n`);
        // Тестируем быструю проверку
        const testPath = path_1.default.resolve('../../../'); // корень eap-analyzer
