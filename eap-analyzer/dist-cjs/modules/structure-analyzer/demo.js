...
function calculateStructureScore(basicResults, advancedResults) {
    ...
    if (basicResults.emptyFiles > 0)
        score -= basicResults.emptyFiles * 2;
    if (advancedResults.duplicationPercentage > 5)
        score -= (advancedResults.duplicationPercentage - 5) * 2;
    if (advancedResults.avgComplexity > 10)
        score -= (advancedResults.avgComplexity - 10) * 3;
    ...
    if (basicResults.testFiles > 0)
        score += Math.min(10, basicResults.testFiles);
    if (basicResults.documentationFiles > 0)
        score += Math.min(5, basicResults.documentationFiles);
...
${roadmap.phases.immediate
        .map((task, index) => `${index + 1}. **${task.title}** (${task.effort}ч)
...
console.log(`   Критических проблем: ${mockRecommendations.filter(r => r.priority === 'high').length}`);
console.log('\n🗺️ ДОРОЖНАЯ КАРТА:');
...
const fullRoadmap = generateRefactoringRoadmap(mockBasicResults, mockAdvancedResults, mockRecommendations);
