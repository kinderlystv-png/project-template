...
exports.EAPStructureIntegration = void 0;
...
class EAPStructureIntegration {
    constructor() {
        this.analyzer = new index_js_1.default();
    }
    /**
    ...
    async saveReport(report, outputPath) {
        try {
            ...
            await promises_1.default.writeFile(outputPath, JSON.stringify(report, null, 2), 'utf8');
            // Сохраняем Markdown дорожную карту
            const markdownPath = outputPath.replace('.json', '-roadmap.md');
            await promises_1.default.writeFile(markdownPath, report.roadmap.content, 'utf8');
    ...
    printSummary(report) {
        ...
        console.log(`💰 ROI: ${report.roadmap.roi.roi}% (окупаемость: ${report.roadmap.roi.paybackPeriod} мес.)`);
        console.log('\n📋 ДОРОЖНАЯ КАРТА:');
...
exports.EAPStructureIntegration = EAPStructureIntegration;
/**
...
async function exampleUsage() {
    ...
    const outputPath = path_1.default.join(projectPath, 'reports', 'structure-analysis.json');
    try {
