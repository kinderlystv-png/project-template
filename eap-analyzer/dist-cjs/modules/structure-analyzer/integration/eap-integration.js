...
class EAPIntegration {
    ...
    generateEAPIntegration(basicResults, advancedResults, recommendations) {
        const structureScore = this.metricsCalculator.calculateStructureScore(basicResults, advancedResults);
        const criticalIssues = recommendations.filter(r => r.priority === 'high').length;
        const maintainabilityMetrics = this.metricsCalculator.calculateMaintainabilityMetrics(basicResults, advancedResults);
        return {
            moduleInfo: {
            ...
            scores: {
                ...
                structuralComplexity: this.metricsCalculator.calculateStructuralComplexity(basicResults, advancedResults),
            },
    ...
    calculateUrgency(structureScore, advancedResults) {
        ...
        if (advancedResults) {
            if (advancedResults.avgMaintainabilityIndex < 50)
                urgencyScore += 30;
            if (advancedResults.duplicationPercentage > 20)
                urgencyScore += 20;
            if (advancedResults.hotspots?.length > 10)
                urgencyScore += 20;
        }
        if (urgencyScore >= 70)
            return 'IMMEDIATE';
    ...
    generateFullEAPReport(projectPath, basicResults, advancedResults, recommendations, analysisTime) {
        ...
        const summary = this.metricsCalculator.generateMetricsSummary(basicResults, advancedResults, recommendations);
        return {
            // Метаданные анализа
            metadata: {
                projectPath,
...
exports.EAPIntegration = EAPIntegration;
exports.default = EAPIntegration;
//# sourceMappingURL=eap-integration.js.map