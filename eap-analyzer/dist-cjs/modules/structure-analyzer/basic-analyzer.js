            ...
            if (config.fileExtensions.includes(ext)) {
                ...
                if (fileStats.needsRefactoring) {
                    ...
                    stats.potentialRefactorFiles.push({
                        path: fullPath,
                        relativePath: path_1.default.relative(process.cwd(), fullPath),
...
async function analyzeFile(filePath, config) {
    ...
    try {
        ...
        if (fileStats.lines > config.thresholds.maxFileSize) {
            ...
            fileStats.refactoringReasons.push(`Large file (${fileStats.lines} lines > ${config.thresholds.maxFileSize})`);
        }
        if (fileStats.complexity > config.thresholds.maxComplexity) {
            fileStats.needsRefactoring = true;
            fileStats.refactoringReasons.push(`High complexity (${fileStats.complexity} > ${config.thresholds.maxComplexity})`);
        }
        // Извлекаем зависимости
        fileStats.dependencies = extractDependencies(content, filePath, config);
        // Анализируем историю Git
        try {
            const { stdout: gitHistory } = await execPromise(`git log --follow --format="%H" -- "${filePath}"`, { timeout: 5000, cwd: path_1.default.dirname(filePath) });
            const commits = gitHistory.trim().split('\n').filter(Boolean);
            fileStats.modificationFrequency = commits.length;
            const { stdout: bugfixCommits } = await execPromise(`git log --follow --grep="fix\\|bug\\|issue\\|resolve\\|close" --format="%H" -- "${filePath}"`, { timeout: 5000, cwd: path_1.default.dirname(filePath) });
...
function resolveImportPath(importPath, filePath, config) {
    try {
        ...
        let resolvedPath = path_1.default.resolve(baseDir, importPath);
...
function calculateMetrics(stats) {
    ...
    stats.criticalIssues += stats.potentialRefactorFiles.filter(f => f.lines > 1000 || f.complexity > 50).length;
...
//# sourceMappingURL=basic-analyzer.js.map