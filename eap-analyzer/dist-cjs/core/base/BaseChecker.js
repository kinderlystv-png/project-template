...
class BaseChecker {
     ...
     */
    constructor(name, category, description, standard, severity = SeverityLevel_1.SeverityLevel.MEDIUM, version = '1.0.0', config = { enabled: true, failOnError: false }) {
        this.name = name;
     ...
     */
    createResult(id, name, description, passed, message, score, maxScore, severity = SeverityLevel_1.SeverityLevel.LOW, details = {}) {
        return {
            id,
            name,
            description,
            passed,
            message,
            severity: passed ? SeverityLevel_1.SeverityLevel.LOW : severity,
            score,
            maxScore,
            timestamp: new Date(),
            details,
     ...
     * @returns Результат проверки
     */
    async safeCheck(project) {
        const startTime = Date.now();
        try {
            if (!this.config.enabled) {
                this.log('Проверщик отключен в конфигурации', 'info');
                return [
                    this.createResult(`${this.name}-disabled`, this.name, this.description, true, 'Проверщик отключен', 1, 1),
                ];
            }
            if (!(await this.isApplicable(project))) {
                ...
                return [
                    this.createResult(`${this.name}-not-applicable`, this.name, this.description, true, message, 1, 1),
                ];
            }
            ...
            if (this.config.failOnError) {
                return [
                    this.createResult(`${this.name}-error`, this.name, this.description, false, `Критическая ошибка: ${errorMessage}`, 0, 1, SeverityLevel_1.SeverityLevel.CRITICAL, { error: errorMessage, duration }),
                ];
            }
            return [
                this.createResult(`${this.name}-error-skipped`, this.name, this.description, true, `Проверка пропущена из-за ошибки: ${errorMessage}`, 0, 1, SeverityLevel_1.SeverityLevel.MEDIUM, { error: errorMessage, duration }),
            ];
        }
    }
    /**
     ...
     */
    async checkFileExists(project, filePath, description = 'файл') {
     ...
     */
    async readJsonSafe(project, filePath) {
        try {
...
//# sourceMappingURL=BaseChecker.js.map