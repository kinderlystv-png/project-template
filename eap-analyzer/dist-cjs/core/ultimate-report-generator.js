...
class UltimateReportGenerator {
    ...
    getProjectStatus(score) {
        if (score >= 90)
            return 'Отлично - готов к production';
        if (score >= 80)
            return 'Хорошо - незначительные улучшения';
        if (score >= 70)
            return 'Удовлетворительно - требует внимания';
        if (score >= 60)
            return 'Требует улучшения - критические проблемы';
        if (score >= 50)
            return 'Плохо - серьезный рефакторинг';
    ...
    extractAIPatterns(results) {
        ...
        Object.entries(results.modules).forEach(([moduleName, moduleResult]) => {
            if (moduleResult.patterns) {
                moduleResult.patterns.forEach((pattern) => {
                    patterns.push({
    ...
    generateQualityPredictions(results) {
    ...
    detectCodeSmells(results) {
        ...
        if (results.modules.complexity) {
            ...
            if (complexityData.hotspots) {
                complexityData.hotspots.forEach((hotspot) => {
                    smells.push({
     ...
     */
    generateArchitectureReport(results) {
     ...
     */
    generateTechnicalDebtReport(results) {
...
private async executeAnalysis(context: CheckContext): Promise<AnalysisResults> {
  ...
  return { checkResults, moduleResults };
     ...
     */
    generateCodeQualityReport(results) {
     ...
     */
    generateComprehensiveRoadmap(results) {
     ...
     */
    generateCompleteVisualizations(results) {
        return {
     ...
     */
    generateEnhancedMetadata(results) {
        return {
            ...results.metadata,
            statistics: {
                filesAnalyzed: 45,
    ...
    generateAntiPatternExample(checkResult) {
        return `// Анти-паттерн обнаружен в: ${checkResult.category}\n// ${checkResult.message}`;
    }
    estimatePatternRefactoringEffort(score) {
        const days = Math.ceil((100 - score) / 20);
        const complexity = days <= 2 ? 'Низкая' : days <= 5 ? 'Средняя' : 'Высокая';
        return { days, complexity };
    }
    generateAIRecommendations(patterns, smells, duplications) {
        const recommendations = [];
        if (patterns.length > 0) {
            recommendations.push(`🔍 Обнаружено ${patterns.length} паттернов. Приоритет: рефакторинг анти-паттернов.`);
        }
        if (smells.length > 0) {
            recommendations.push(`👃 Найдено ${smells.length} code smells. Начните с критических.`);
        }
        if (duplications.percentage > 10) {
            recommendations.push(`📋 Дублирование ${duplications.percentage}%. Потенциальная экономия: ${duplications.savings} часов.`);
        }
    ...
    calculateAIConfidence(results) {
        ...
        const confidence = Math.min(90, dataQuality * 0.4 + Math.min(moduleCount * 10, 30) + Math.min(checkCount * 5, 30));
        return Math.round(confidence);
    }
}
exports.UltimateReportGenerator = UltimateReportGenerator;
//# sourceMappingURL=ultimate-report-generator.js.map