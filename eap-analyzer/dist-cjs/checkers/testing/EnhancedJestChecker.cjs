...
class EnhancedJestChecker extends BaseChecker {
  ...
  constructor() {
    ...
    this.testPatterns = ['describe', 'test', 'it', 'beforeEach', 'afterEach', 'beforeAll', 'afterAll'];
  }
  ...
  async check(context) {
    ...
    try {
      ...
      return this.createResult(false, 0, `Ошибка анализа Jest: ${error.message}`, 
        { error: error.message }, ['Проверьте структуру проекта и права доступа']);
    }
  }
  ...
  async analyzeJestConfiguration(projectPath) {
    const configFiles = [
      ...
      'jest.config.json', 'jest.config.cjs'
    ];
  ...
  async analyzeTestFiles(projectPath) {
    ...
    for (const testFile of testFiles.slice(0, 5)) { // Анализируем первые 5 файлов
      try {
  ...
  analyzeTestFileContent(content) {
    ...
    analysis.hasAsyncTests = content.includes('async ') && (content.includes('await ') || content.includes('Promise'));
  ...
  async analyzeMockPatterns(projectPath) {
    ...
    for (const testFile of testFiles.slice(0, 10)) { // Анализируем до 10 файлов
      try {
        const content = await fs.readFile(testFile, 'utf8');
        let fileMockCount = 0;
    ...
    if (totalFilesAnalyzed > 0) {
      ...
      mockAnalysis.mockQualityScore = Math.min(100, (mockCoverage * 60) + (Math.min(avgMocksPerFile, 5) * 8));
    }
  ...
  async analyzeCoverageSetup(projectPath) {
    ...
    if (!analysis.hasCoverageConfig) {
      ...
      if (await this.fileExists(packageJsonPath)) {
        ...
        if (packageJson.jest && (packageJson.jest.collectCoverage || packageJson.jest.coverageThreshold)) {
          analysis.hasCoverageConfig = true;
          if (packageJson.jest.coverageThreshold && packageJson.jest.coverageThreshold.global) {
            ...
            analysis.coverageThreshold = Math.max(global.branches || 0, global.functions || 0, global.lines || 0, global.statements || 0);
          }
        }
    ...
    analysis.coverageScore = analysis.hasCoverageConfig ? 
      (analysis.coverageThreshold ? Math.min(100, 50 + analysis.coverageThreshold/2) : 50) : 0;
  ...
  async findTestFiles(projectPath) {
    ...
    const extensions = ['.test.js', '.spec.js', '.test.ts', '.spec.ts', '.test.jsx', '.spec.jsx', '.test.tsx', '.spec.tsx'];
  ...
  generateEnhancedRecommendations(details) {
    ...
    if (details.mockQualityScore < 30) {
      recommendations.push('Рассмотрите использование моков для изоляции тестов (jest.mock, jest.spyOn)');
    }
