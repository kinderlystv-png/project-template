...
exports.PackageJsonAnalyzer = void 0;
...
class PackageJsonAnalyzer {
   ...
   */
  async analyzePackageJson(project) {
    try {
  ...
  detectFrameworks(dependencies) {
    ...
    constants_1.SUPPORTED_FRAMEWORKS.UNIT_TESTING.forEach(frameworkName => {
    ...
    constants_1.SUPPORTED_FRAMEWORKS.E2E_TESTING.forEach(frameworkName => {
      if (dependencyNames.includes(frameworkName)) {
        frameworks.push({
          name: frameworkName,
          version: this.extractVersion(dependencies[frameworkName]),
    ...
    constants_1.SUPPORTED_FRAMEWORKS.COVERAGE_TOOLS.forEach(toolName => {
   ...
   */
  async hasCoverageScripts(project) {
    try {
      const packageJsonPath = path.join(project.path, 'package.json');
      if (!(await this.fileExists(packageJsonPath))) {
        return false;
   ...
   */
  getRecommendedScripts(frameworks) {
    const scripts = {};
