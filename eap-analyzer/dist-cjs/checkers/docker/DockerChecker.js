 ...
 */
class DockerChecker extends BaseChecker_1.BaseChecker {
    constructor() {
        super('DockerChecker', AnalysisCategory_1.AnalysisCategory.INFRASTRUCTURE, 'Проверка Docker инфраструктуры и контейнеризации', 'Docker Best Practices', SeverityLevel_1.SeverityLevel.HIGH, '2.0.0', {
            enabled: true,
            failOnError: false,
            thresholds: {
                min_score: 70,
        ...
        return [
            {
                ...
                maxScore: 15,
                tags: [AnalysisCategory_1.AnalysisCategory.INFRASTRUCTURE, 'containerization'],
            },
            {
                id: 'docker-compose',
                ...
                maxScore: 15,
                tags: [AnalysisCategory_1.AnalysisCategory.INFRASTRUCTURE, 'orchestration'],
            },
            {
                ...
                maxScore: 10,
                tags: [AnalysisCategory_1.AnalysisCategory.INFRASTRUCTURE, 'optimization'],
            },
            {
                ...
                severity: SeverityLevel_1.SeverityLevel.HIGH,
                maxScore: 15,
                tags: [AnalysisCategory_1.AnalysisCategory.INFRASTRUCTURE, 'production'],
            },
            {
                id: 'docker-security',
                ...
                severity: SeverityLevel_1.SeverityLevel.HIGH,
                maxScore: 10,
                tags: [AnalysisCategory_1.AnalysisCategory.SECURITY, 'container'],
            },
            {
                ...
                severity: SeverityLevel_1.SeverityLevel.MEDIUM,
                maxScore: 10,
                tags: [AnalysisCategory_1.AnalysisCategory.INFRASTRUCTURE, 'monitoring'],
            },
            {
                id: 'docker-ignore',
                name: 'Docker Ignore',
            ...
            },
            {
                ...
                severity: SeverityLevel_1.SeverityLevel.MEDIUM,
                maxScore: 10,
                tags: [AnalysisCategory_1.AnalysisCategory.INFRASTRUCTURE, 'environments'],
            },
        ];
    }
    /**
     ...
     */
    async executeIndividualCheck(checkInfo, project) {
        try {
            switch (checkInfo.id) {
                ...
                default:
                    return this.createResult(checkInfo.id, checkInfo.name, checkInfo.description, false, 'Неизвестная проверка', 0, checkInfo.maxScore, SeverityLevel_1.SeverityLevel.LOW);
            }
        }
        catch (error) {
            const errorMessage = error instanceof Error ? error.message : String(error);
            return this.createResult(checkInfo.id, checkInfo.name, checkInfo.description, false, `Ошибка выполнения проверки: ${errorMessage}`, 0, checkInfo.maxScore, SeverityLevel_1.SeverityLevel.CRITICAL, { error: errorMessage });
        }
    }
    /**
     * Проверка Dockerfile
     */
    async checkDockerfile(checkInfo, project) {
        if (!(await project.exists('Dockerfile'))) {
            return this.createResult(checkInfo.id, checkInfo.name, checkInfo.description, false, 'Dockerfile не найден', 0, checkInfo.maxScore, SeverityLevel_1.SeverityLevel.CRITICAL, { recommendations: ['Создайте Dockerfile для контейнеризации приложения'] });
        }
        const content = await project.readFile('Dockerfile');
        if (!content.trim()) {
            return this.createResult(checkInfo.id, checkInfo.name, checkInfo.description, false, 'Dockerfile пуст', 0, checkInfo.maxScore, SeverityLevel_1.SeverityLevel.CRITICAL);
        }
        ...
        return this.createResult(checkInfo.id, checkInfo.name, checkInfo.description, passed, `Качество Dockerfile: ${score}/6 элементов (${Math.round(percentage)}%)`, finalScore, checkInfo.maxScore, passed ? SeverityLevel_1.SeverityLevel.LOW : SeverityLevel_1.SeverityLevel.HIGH, {
            elements: {
                FROM: hasFrom,
                WORKDIR: hasWorkdir,
                'COPY/ADD': hasCopy,
     ...
     */
    async checkDockerCompose(checkInfo, project) {
        const composeFiles = [
            'docker-compose.yml',
            'docker-compose.yaml',
            'compose.yml',
        ...
        return this.createResult(checkInfo.id, checkInfo.name, checkInfo.description, passed, passed
            ? `Найдено ${foundFiles} compose файлов с ${foundServices} сервисами`
            : 'Docker Compose конфигурация не найдена', score, checkInfo.maxScore, passed ? SeverityLevel_1.SeverityLevel.LOW : SeverityLevel_1.SeverityLevel.HIGH, {
            foundFiles,
            foundServices,
            files: details,
     ...
     */
    async checkMultiStage(checkInfo, project) {
        if (!(await project.exists('Dockerfile'))) {
            return this.createResult(checkInfo.id, checkInfo.name, checkInfo.description, false, 'Dockerfile не найден для проверки multi-stage', 0, checkInfo.maxScore, SeverityLevel_1.SeverityLevel.MEDIUM);
        }
        const content = await project.readFile('Dockerfile');
        ...
        return this.createResult(checkInfo.id, checkInfo.name, checkInfo.description, isMultiStage, isMultiStage
            ? `Multi-stage сборка обнаружена (${fromMatches.length} этапов)`
            : 'Multi-stage сборка не используется', score, checkInfo.maxScore, isMultiStage ? SeverityLevel_1.SeverityLevel.LOW : SeverityLevel_1.SeverityLevel.MEDIUM, {
     ...
     */
    async checkProductionReady(checkInfo, project) {
        const prodFiles = [
        ...
        const score = passed ? checkInfo.maxScore : 0;
        return this.createResult(checkInfo.id, checkInfo.name, checkInfo.description, passed, `Production готовность: файлы (${foundProdFiles}), ENV конфигурация (${hasEnvConfig})`, score, checkInfo.maxScore, passed ? SeverityLevel_1.SeverityLevel.LOW : SeverityLevel_1.SeverityLevel.HIGH, {
     ...
     */
    async checkSecurity(checkInfo, project) {
        if (!(await project.exists('Dockerfile'))) {
            return this.createResult(checkInfo.id, checkInfo.name, checkInfo.description, false, 'Dockerfile не найден для проверки безопасности', 0, checkInfo.maxScore, SeverityLevel_1.SeverityLevel.HIGH);
        }
        const content = await project.readFile('Dockerfile');
        ...
        const score = passed
            ...
            : 0;
        return this.createResult(checkInfo.id, checkInfo.name, checkInfo.description, passed, `Безопасность: ${securityScore}/4 функций`, score, checkInfo.maxScore, passed ? SeverityLevel_1.SeverityLevel.LOW : SeverityLevel_1.SeverityLevel.HIGH, {
            securityFeatures: {
                nonRootUser: hasUser,
     ...
     */
    async checkHealthCheck(checkInfo, project) {
        if (!(await project.exists('Dockerfile'))) {
            return this.createResult(checkInfo.id, checkInfo.name, checkInfo.description, false, 'Dockerfile не найден для проверки health check', 0, checkInfo.maxScore, SeverityLevel_1.SeverityLevel.MEDIUM);
        }
        const content = await project.readFile('Dockerfile');
        const hasHealthCheck = content.includes('HEALTHCHECK');
        const score = hasHealthCheck ? checkInfo.maxScore : 0;
        return this.createResult(checkInfo.id, checkInfo.name, checkInfo.description, hasHealthCheck, hasHealthCheck ? 'HEALTHCHECK настроен' : 'HEALTHCHECK не найден', score, checkInfo.maxScore, hasHealthCheck ? SeverityLevel_1.SeverityLevel.LOW : SeverityLevel_1.SeverityLevel.MEDIUM, {
            hasHealthCheck,
     ...
     */
    async checkDockerIgnore(checkInfo, project) {
        const hasDockerIgnore = await project.exists('.dockerignore');
        const score = hasDockerIgnore ? checkInfo.maxScore : 0;
        return this.createResult(checkInfo.id, checkInfo.name, checkInfo.description, hasDockerIgnore, hasDockerIgnore ? '.dockerignore найден' : '.dockerignore отсутствует', score, checkInfo.maxScore, hasDockerIgnore ? SeverityLevel_1.SeverityLevel.LOW : SeverityLevel_1.SeverityLevel.LOW, // Не критично
        {
            hasDockerIgnore,
     ...
     * Проверка окружений
     */
    async checkEnvironments(checkInfo, project) {
        const envFiles = [
        ...
        return this.createResult(checkInfo.id, checkInfo.name, checkInfo.description, passed, `Окружения: ${foundEnvFiles} из ${envFiles.length}`, score, checkInfo.maxScore, passed ? SeverityLevel_1.SeverityLevel.LOW : SeverityLevel_1.SeverityLevel.MEDIUM, {
            foundEnvFiles,
...
//# sourceMappingURL=DockerChecker.js.map