'use strict';
/**
 * SecurityChecker - Интегрированный анализатор безопасности
 *
 * Phase 5.1: Восстановление SecurityChecker с интеграцией в ProcessIsolatedAnalyzer
 *
 * Использует ProcessIsolatedAnalyzer для безопасного запуска
 * анализаторов безопасности в изолированном процессе
 */
...
const DependenciesSecurityChecker_js_1 = require('./DependenciesSecurityChecker.js');
const CodeSecurityChecker_js_1 = require('./CodeSecurityChecker.js');
const ConfigSecurityChecker_js_1 = require('./ConfigSecurityChecker.js');
const WebSecurityChecker_js_1 = require('./WebSecurityChecker.js');
class SecurityChecker {
  static analyzer = new ProcessIsolatedAnalyzer_js_1.ProcessIsolatedAnalyzer();
  static dependenciesChecker = new DependenciesSecurityChecker_js_1.DependenciesSecurityChecker();
  static codeChecker = new CodeSecurityChecker_js_1.CodeSecurityChecker();
  static configChecker = new ConfigSecurityChecker_js_1.ConfigSecurityChecker();
  static webSecurityChecker = new WebSecurityChecker_js_1.WebSecurityChecker();
  /**
   ...
   */
  static async runSecurityAnalysis(context) {
    try {
   ...
   */
  static calculateOverallScore(deps, code, config, webSecurity) {
    let score = 100;
    ...
    if (code.issues) {
      const criticalCodeIssues = code.issues.filter(i => i.severity === 'critical').length;
      const highCodeIssues = code.issues.filter(i => i.severity === 'high').length;
      score -= criticalCodeIssues * 15;
      score -= highCodeIssues * 8;
    }
    // Снижаем балл за проблемы конфигурации
    if (config.issues) {
      const criticalConfigIssues = config.issues.filter(i => i.severity === 'critical').length;
      const highConfigIssues = config.issues.filter(i => i.severity === 'high').length;
    ...
    if (webSecurity && webSecurity.summary) {
      ...
      const criticalWebIssues =
        webSecurity.xss?.vulnerabilities?.filter(v => v.severity === 'critical').length || 0;
      const highWebIssues =
        webSecurity.xss?.vulnerabilities?.filter(v => v.severity === 'high').length || 0;
   ...
   */
  static createErrorResult(startTime, error) {
    const errorCheck = {
      check: {
        id: 'security-error',
  ...
  static calculateDependenciesScore(deps) {
    let score = 100;
    if (deps.auditSummary) {
  ...
  static calculateCodeScore(code) {
    let score = 100;
    if (code.issues) {
      const critical = code.issues.filter(i => i.severity === 'critical').length;
      const high = code.issues.filter(i => i.severity === 'high').length;
      const medium = code.issues.filter(i => i.severity === 'medium').length;
      score -= critical * 20;
      score -= high * 10;
      score -= medium * 5;
    }
  ...
  static calculateConfigScore(config) {
    let score = 100;
    if (config.issues) {
      const critical = config.issues.filter(i => i.severity === 'critical').length;
      const high = config.issues.filter(i => i.severity === 'high').length;
      score -= critical * 15;
      score -= high * 8;
    }
    return Math.max(0, score);
  }
  static calculateWebSecurityScore(webSecurity) {
    let score = 100;
    if (!webSecurity || !webSecurity.summary) {
    ...
    const criticalXSS = xssVulns.filter(v => v.severity === 'critical').length;
    const highXSS = xssVulns.filter(v => v.severity === 'high').length;
    const mediumXSS = xssVulns.filter(v => v.severity === 'medium').length;
    score -= criticalXSS * 20;
    score -= highXSS * 12;
    score -= mediumXSS * 6;
    // Снижаем балл за CSRF проблемы
    const criticalCSRF = csrfIssues.filter(i => i.severity === 'high').length;
    const mediumCSRF = csrfIssues.filter(i => i.severity === 'medium').length;
    score -= criticalCSRF * 15;
    score -= mediumCSRF * 8;
    return Math.max(0, score);
  }
  // Helper methods для рекомендаций
  static getDependenciesRecommendations(deps) {
  ...
  static getCodeRecommendations(code) {
    const recommendations = [];
  ...
  static getWebSecurityRecommendations(webSecurity) {
    const recommendations = [];
    if (!webSecurity || !webSecurity.summary) {
    ...
    if (xssVulns.length > 0) {
      ...
      const dangerousHTML = xssVulns.filter(v => v.type === 'dangerous_html').length;
      const unsafeJS = xssVulns.filter(v => v.type === 'unsafe_js_execution').length;
      if (dangerousHTML > 0) {
    ...
    if (csrfIssues.length > 0) {
      ...
      const unprotectedForms = csrfIssues.filter(i => i.type === 'unprotected_form').length;
      const cookieIssues = csrfIssues.filter(i => i.type === 'cookie_security').length;
  ...
  static generateRecommendations(failed) {
   ...
   */
  static async generateDetailedRecommendations(context) {
    try {
   ...
   */
  static async generateQuickRecommendations(context) {
    try {
      const analysisResult = await this.runSecurityAnalysis(context);
   ...
   */
  static calculateEstimatedTime(recommendations) {
    let totalMinutes = 0;
