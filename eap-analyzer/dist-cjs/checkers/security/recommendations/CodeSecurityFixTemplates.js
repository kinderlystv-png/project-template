...
exports.CodeSecurityFixTemplates = void 0;
class CodeSecurityFixTemplates {
  /**
...
// 4. Санитизируйте входы`,
  ...
  static getSecretBeforeExample(type, pattern) {
    const examples = {
      api_key: `// ОПАСНО ❌:
...
fetch('/api/data', {
  ...
  static getSecretAfterExample(type) {
    const examples = {
      api_key: `// БЕЗОПАСНО ✅:
...
AUTH_TOKEN=your_auth_token_here`,
  ...
  static getSecretImplementationExample(type) {
...
// - Cloud provider secret management (AWS Secrets Manager, etc.)`;
  ...
  static getUnsafeFunctionBefore(functionName) {
    const examples = {
      eval: `// ОПАСНО ❌:
...
userFunction(); // Аналог eval`,
  ...
  static getUnsafeFunctionAfter(functionName) {
    const examples = {
      eval: `// БЕЗОПАСНО ✅:
try {
...
// Используйте готовые функции или безопасные альтернативы`,
  ...
  static getUnsafeFunctionExplanation(functionName) {
    const explanations = {
  ...
  static getSQLInjectionBefore(type, query) {
    const examples = {
...
db.query(query); // SQL injection!`,
  ...
  static getSQLInjectionAfter(type) {
    const examples = {
...
document.getElementById('content').textContent = hash; // Только текст`,
  ...
  static getUnsafeFunctionSeverity(functionName) {
  ...
  static getUnsafeFunctionFixTime(functionName) {
    const timeMap = {
      eval: '2-4 часа',
  ...
  static getUnsafeFunctionDifficulty(functionName) {
...
exports.CodeSecurityFixTemplates = CodeSecurityFixTemplates;
//# sourceMappingURL=CodeSecurityFixTemplates.js.map
