...
exports.DependenciesChecker = void 0;
...
class DependenciesChecker {
    ...
    async checkPackageManager() {
        ...
        if (passed) {
            ...
            const npmrcPath = (0, path_1.join)(this.context.projectPath, '.npmrc');
    ...
    async checkLockFile() {
        ...
        if (passed) {
            try {
                const stats = require('fs').statSync((0, path_1.join)(this.context.projectPath, lockFile));
                const sizeKB = Math.round(stats.size / 1024);
                details = `Lock файл: ${lockFile} (${sizeKB} KB)`;
                // Проверим актуальность (должен быть не старше package.json)
                const packageStats = require('fs').statSync((0, path_1.join)(this.context.projectPath, 'package.json'));
    ...
    async checkPackageJsonStructure() {
        ...
        if ((0, fs_1.existsSync)(packagePath)) {
            try {
                ...
                const foundDepTypes = depTypes.filter(type => pkg[type] && Object.keys(pkg[type]).length > 0);
        ...
        if ((0, fs_1.existsSync)(packagePath)) {
            try {
                ...
                if (Object.keys(allDeps).length > 0) {
                    ...
                    // Статистика версий
                    if (exactVersions > 0)
                        details += `. Точные: ${exactVersions}`;
                    if (caretVersions > 0)
                        details += `. Caret (^): ${caretVersions}`;
                    if (tildeVersions > 0)
                        details += `. Tilde (~): ${tildeVersions}`;
                    if (latestVersions > 0)
                        details += `. Latest: ${latestVersions}`;
                    if (otherVersions > 0)
                        details += `. Другие: ${otherVersions}`;
    ...
    async checkDependencyUpdates() {
        ...
        const renovatePath = (0, path_1.join)(this.context.projectPath, 'renovate.json');
        let passed = false;
        let details = '';
        // Проверим Dependabot
        if ((0, fs_1.existsSync)(dependabotPath)) {
            passed = true;
        ...
        // Проверим Renovate
        if ((0, fs_1.existsSync)(renovatePath)) {
            if (!passed) {
                passed = true;
        ...
        if ((0, fs_1.existsSync)(packagePath)) {
            try {
                ...
                const devDeps = pkg.devDependencies || {};
                const scripts = pkg.scripts || {};
                const hasUpdateTools = devDeps['npm-check-updates'] ||
    ...
    async checkBundleAnalysis() {
        ...
        if ((0, fs_1.existsSync)(packagePath)) {
            try {
                ...
                const hasAnalyzeScript = scripts.analyze || scripts['bundle:analyze'] || scripts['build:analyze'];
                if (hasAnalyzeScript) {
                    if (!passed) {
                        passed = true;
    ...
    async checkWorkspaceConfig() {
        const packagePath = (0, path_1.join)(this.context.projectPath, 'package.json');
        const pnpmWorkspacePath = (0, path_1.join)(this.context.projectPath, 'pnpm-workspace.yaml');
        let passed = false;
        let details = '';
        ...
        if ((0, fs_1.existsSync)(pnpmWorkspacePath)) {
            ...
            try {
                const content = (0, fs_1.readFileSync)(pnpmWorkspacePath, 'utf-8');
...
exports.DependenciesChecker = DependenciesChecker;
