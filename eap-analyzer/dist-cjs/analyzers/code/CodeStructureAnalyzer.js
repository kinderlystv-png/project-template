...
class CodeStructureAnalyzer extends interfaces_1.BaseAnalyzer {
    LARGE_FILE_THRESHOLD = 1000000; // 1MB в байтах
    MAX_DIRECTORY_FILES = 50; // Максимальное количество файлов в директории
    constructor() {
        super('CodeStructureAnalyzer', 'Анализирует структуру кода, распределение файлов и модульность проекта');
    }
    async analyze(project) {
        ...
        try {
            ...
            const sourceFiles = allFiles.filter(file => this.isSourceCodeFile(file) && !this.isIgnoredDirectory(file));
            ...
            console.log(`[${this.name}] Анализ завершен. Найдено ${sourceFiles.length} файлов исходного кода`);
            return result;
        }
        catch (error) {
     ...
     */
    async analyzeFileSizes(project, files) {
        const fileSizes = {};
     ...
     */
    async calculateModularity(project, files) {
        let modularity = 0;
        let totalChecks = 0;
...
//# sourceMappingURL=CodeStructureAnalyzer.js.map