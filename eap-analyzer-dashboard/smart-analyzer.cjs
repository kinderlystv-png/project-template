...
// ===================================================================
// ФУНКЦИЯ ДЕТЕКЦИИ ДУБЛИРОВАНИЯ КОДА
// ===================================================================

function detectCodeDuplication(content) {
  ...
  const duplicationRatio = duplicateLines / lines.length;
...
function determineComponentType(filename, filepath, content) {
  ...
  if (filename.includes('BenchUtils') ||
  ...
  for (const [type, config] of Object.entries(COMPONENT_TYPES)) {
    if (config.patterns?.some(pattern => pattern.test(lowerFilename) || pattern.test(lowerFilepath))) {
      return type;
    }
  }
  ...
  if (content.length < 1000) {
    if (contentLower.includes('class') && (contentLower.includes('service') || contentLower.includes('user'))) {
      return 'SERVICE';
    }
...
function detectImprovements(content, componentType) {
  ...
  const duplicateScore = detectCodeDuplication(content);
