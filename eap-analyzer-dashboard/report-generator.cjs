...
class EAPReportGenerator {
    constructor(options = {}) {
        ...
        this.outputPath = options.outputPath || path.join(this.projectRoot, 'eap-analyzer-dashboard', 'data', 'reports');
    ...
    async generateReport() {
        ...
        try {
            ...
            console.log(`📈 Средняя готовность: ${statistics.avgLogic.toFixed(1)}% / ${statistics.avgFunctionality.toFixed(1)}%`);
    ...
    analyzeCode(content) {
        ...
        const functionMatches = content.match(/(function\s+\w+|const\s+\w+\s*=\s*\(?|class\s+\w+|async\s+function)/g) || [];
    ...
    determineCategory(filePath, componentName) {
        ...
        if (pathLower.includes('test') || nameLower.includes('test') || nameLower.includes('jest')) return 'testing';
        if (pathLower.includes('security') || nameLower.includes('security')) return 'security';
        if (pathLower.includes('performance') || nameLower.includes('performance')) return 'performance';
    ...
    calculateStatistics(components) {
        ...
        if (totalComponents === 0) {
            return { total: 0, avgLogic: 0, avgFunctionality: 0, ready: 0, inProgress: 0, planned: 0, categories: {} };
        }
        ...
        let ready = 0, inProgress = 0, planned = 0;
        ...
        return { total: totalComponents, avgLogic, avgFunctionality, ready, inProgress, planned, categories };
    }
