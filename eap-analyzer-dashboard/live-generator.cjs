...
function analyzeProjectFiles(projectPath = '../eap-analyzer') {
  ...
  function analyzeFile(filename, filepath, content, stat) {
    try {
      ...
      // Генерируем ключевые недостатки (от 1 до 5 проблем для критичных случаев)
      const logicIssues = identifyMultipleLogicIssues(content, category, filepath, logic);
      const functionalityIssues = identifyMultipleFunctionalityIssues(content, category, filepath, functionality);
