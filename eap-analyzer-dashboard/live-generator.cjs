...
const { smartComponentAnalyzer, isAutoGenerated } = require('./smart-analyzer.cjs');
...
function analyzeProjectFiles(projectPath = '../eap-analyzer') {
  ...
  function analyzeFile(filename, filepath, content, stat) {
    try {
      // НОВАЯ ФИЛЬТРАЦИЯ: Проверяем автогенерированные файлы
      if (isAutoGenerated(filepath, content)) {
        return null; // Пропускаем автогенерированные файлы
      }
      ...
      return {
        name: componentName,
        category,
        type: smartAnalysis.type, // Добавляем тип из УАК
        logic,
        functionality,
        ...
        // Добавляем данные об улучшениях из УАК
        improvements: smartAnalysis.improvements || {},
        sizeAdjustment: smartAnalysis.sizeAdjustment || 1.0,
      };
    } catch (error) {
