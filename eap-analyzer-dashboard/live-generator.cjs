...
function analyzeProjectFiles(projectPath = '../eap-analyzer') {
  ...
  function analyzeFile(filename, filepath, content, stat) {
    try {
      ...
      // Генерируем ключевые недостатки
      const logicIssue = identifyLogicIssue(content, category, filepath);
      const functionalityIssue = identifyFunctionalityIssue(content, category, filepath);
...
function generateDataJs(data) {
  ...
  const jsHistory = {
    [today]: {
      avgLogic: Math.round(Object.values(components).reduce((sum, c) => sum + c.logic, 0) / Object.keys(components).length),
      avgFunctionality: Math.round(Object.values(components).reduce((sum, c) => sum + c.functionality, 0) / Object.keys(components).length),
...
function main() {
  ...
  // Генерируем обновленный data.js с недостатками
  const jsDataContent = generateDataJs(data);
