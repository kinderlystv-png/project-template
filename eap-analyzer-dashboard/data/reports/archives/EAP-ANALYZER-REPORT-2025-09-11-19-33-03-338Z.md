...
- **AdvancedSecurityAnalyzer** [73% / 67%] - Расширенный анализатор безопасности для Task 2.3   Добавляет +10 новых типов угроз и улучшает покры
- **DockerChecker** [72% / 66%] - Проверки для Docker инфраструктуры
- **CodeSecurityChecker** [72% / 66%] - Code Security Checker - Enhanced for Task 2.3   Статический анализ кода на предмет проблем безопасн
- **BaseChecker** [72% / 65%] - Базовый класс для всех чекеров системы
- **CSRFAnalyzer** [71% / 66%] - CSRFAnalyzer - Анализатор CSRF уязвимостей     Обнаруживает отсутствие защиты от Cross-Site Reques
- **XSSAnalyzer** [71% / 65%] - XSSAnalyzer - Анализатор XSS уязвимостей     Обнаруживает потенциальные Cross-Site Scripting уязви
- **ConfigSecurityChecker** [71% / 65%] - Config Security Checker   Анализ безопасности конфигурационных файлов
- **DependenciesSecurityChecker** [71% / 65%] - Dependencies Security Checker   Анализ безопасности зависимостей проекта     Phase 5.2.1: Интегра
- **DependenciesChecker** [70% / 66%] - Dependencies Management Checker   Проверки управления зависимостями и пакетным менеджером
- **SecurityChecker** [70% / 65%] - Чекер безопасности - проверяет общие проблемы безопасности
- **WebSecurityChecker** [70% / 65%] - WebSecurityChecker - Главный анализатор веб-безопасности     Координирует анализ XSS, CSRF и други
- **WebSecurityFixTemplates** [70% / 65%] - WebSecurityFixTemplates - Шаблоны исправлений для веб-уязвимостей     Phase 5.2.2 - Задача 1.2   
- **DockerSecurityChecker** [70% / 64%] - Docker Security Checker - проверяет безопасность Docker конфигураций
- **EMTDependenciesChecker** [70% / 65%] - EMT Dependencies Checker - проверяет зависимости EMT проекта
- **AnalysisManager** [70% / 66%] - Модуль управления анализом проекта   Координирует базовый и расширенный анализ
- **CICDChecker** [69% / 65%] - CI/CD Pipeline Checker   Проверки настройки непрерывной интеграции и развертывания
- **AnalysisReportAdapter** [69% / 64%] - EAP Analysis to Report Data Adapter   Адаптер для преобразования результатов анализа EAP в данные д
- **ROADMAPANALYSISmd** [66% / 63%] - EAP Analyzer компонент
- **Class** [66% / 58%] - Benchmark generation script for EAP Analyzer   Generates performance benchmarks and comparison repo
- **AnalysisCategoryts** [66% / 62%] - Категории анализа в EAP системе
- **analysisresultsts** [65% / 58%] - Analysis Results Types   Типы данных для результатов анализа различных аспектов проекта
- **SmartFileClassifier** [62% / 61%] - Система умной классификации файлов   Определяет тип файла и его назначение для корректного анализа
- **GoldenStandardAnalyzer** [61% / 60%] - Эталонный Анализатор Проектов (ЭАП)   Главный класс для анализа проектов по золотому стандарту   В
- **TemplateManager** [61% / 59%] - Template Manager - Менеджер шаблонов   Связующее звено между Registry, Loader и TemplateEngine   О
- **md** [60% / 60%] - Импортируем все модули анализатора
- **BaseEvaluator** [60% / 59%] - Base Evaluator Interface   Базовый интерфейс для всех оценщиков
...
- **ImprovedComplexityCalculator** [59% / 58%] - Улучшенный калькулятор цикломатической и когнитивной сложности   Исправляет неадекватные значения с
...
### 6. 🧪 **TESTING (Тестирование)** [58% / 50%]

- **JestChecker** [63% / 55%] - JestChecker - специализированный анализатор для Jest фреймворка   Проверяет конфигурацию, тесты и н
- **PlaywrightChecker** [63% / 56%] - PlaywrightChecker - анализатор Playwright E2E тестирования   Проверяет конфигурацию, тесты и настро
- **PlaywrightCheckerAdapter** [63% / 55%] - Адаптер PlaywrightChecker для интеграции с AnalysisOrchestrator   Анализирует конфигурацию и настро
- **CoverageAnalyzer** [62% / 53%] - CoverageAnalyzer - анализатор покрытия кода тестами   Анализирует отчеты покрытия и предоставляет д
- **CypressChecker** [62% / 54%] - CypressChecker - анализатор Cypress E2E тестирования   Проверяет конфигурацию, тесты и настройки Cy
- **E2ETestingChecker** [62% / 54%] - E2ETestingChecker - универсальный анализатор E2E тестирования   Координирует анализ различных E2E ф
- **QualityMetricsAnalyzer** [62% / 55%] - QualityMetricsAnalyzer - анализатор качества и метрик тестирования   Предоставляет комплексный анал
- **TestFileFinder** [62% / 53%] - Утилиты для поиска и анализа тестовых файлов
- **ProductModel** [62% / 53%] - Тесты для AI Pattern Recognizer   Проверяем обнаружение различных архитектурных паттернов и антипат
- **performancesetupts** [62% / 55%] - Performance Testing Setup   Инициализация системы отчетов и автоматизации     @author EAP Analyze
- **HtmlReportGenerator** [62% / 56%] - Performance Report Generator   Автоматизация создания HTML отчетов и уведомлений     @author EAP 
- **MockProcessIsolatedAnalyzer** [61% / 53%] - Упрощенный EAP анализатор для тестирования TestingChecker   Обходит проблемы компиляции других комп
- **CoverageAnalyzerAdapter** [61% / 53%] - Адаптер CoverageAnalyzer для интеграции с AnalysisOrchestrator
- **CypressCheckerAdapter** [61% / 53%] - Адаптер CypressChecker для интеграции с AnalysisOrchестратором   Анализирует конфигурацию и настрой
- **VitestChecker** [61% / 56%] - Vitest Testing Framework Checker   Проверки конфигурации и настройки Vitest
- **CoverageChecker** [61% / 53%] - Конфигурация покрытия тестами
- **RoadmapReporter** [61% / 60%] - Roadmap Reporter   Специализированный репортер для создания дорожной карты развития проекта
- **IntegrationTester** [61% / 53%] - Интеграционные тесты для проверки всего цикла валидации   Тестирует взаимодействие всех компонентов
- **COMPONENTSPECIFICATIONmd** [60% / 66%] - EAP Analyzer компонент
- **MockProject** [60% / 54%] - Mock Project implementation
- **TestingAnalysisAdapter** [60% / 52%] - TestingAnalysisAdapter - JavaScript версия для тестирования   Phase 4.1: Адаптер для интеграции с A
- **TestingChecker** [60% / 52%] - TestingChecker - Интегрированный анализатор тестирования     Phase 4.2: Интеграция UnifiedTestingA
- **JestCheckerAdapter** [60% / 52%] - Адаптер JestChecker для интеграции с AnalysisOrchestrator
- **E2EChecker** [60% / 52%] - Информация о E2E фреймворке
- **TestingFrameworkChecker** [60% / 52%] - Информация о тестовом фреймворке
- **TestingTypests** [60% / 53%] - Типы для модуля тестирования   Описывают структуры данных для обмена информацией между чекерами
- **ConfigFileAnalyzer** [60% / 52%] - Анализатор конфигурационных файлов тестовых фреймворков
- **PackageJsonAnalyzer** [60% / 52%] - Анализатор package.json для тестовых фреймворков и конфигураций
- **testts** [60% / 52%] - Тестовый запуск ЭАП анализатора
- **BugFixTester** [60% / 51%] - Модуль для тестирования исправлений критических багов EAP   Проверяет правильность расчета дупликац
- **testplaywrightsimple** [60% / 54%] - Простой тест PlaywrightChecker без TypeScript (для быстрого запуска)
- **aiinsightsenginetestts** [60% / 52%] - Тесты для AI Insights Engine - центрального движка AI анализа
...
- **testbothframeworksts** [58% / 50%] - Объединенный тест VitestChecker и JestChecker с SimpleOrchestrator
- **testphase3final** [58% / 51%] - Phase 3 Test - Финальный тест комплексного анализа тестовой экосистемы   Использует предыдущие тест
- **testrecommendationfinalts** [58% / 51%] - Тест интеграции веб-безопасности с RecommendationEngine   Проверяет полный поток: WebSecurityChecke
- **testwebintegrationts** [58% / 50%] - Тест интеграции WebSecurityChecker с основным SecurityChecker     Проверяет что веб-анализ безопас
- **dockeranalyzertestts** [58% / 50%] - Unit тесты для Docker Analyzer
- **eapsetupts** [58% / 50%] - Setup файл для Vitest тестов
- **FileSystemProject** [58% / 50%] - Интеграционный тест для новой архитектуры ЭАП   Демонстрирует работу DockerChecker с реальными прим
- **executiontimertestts** [58% / 52%] - Тесты для Execution Timer   Проверяет корректность измерения времени выполнения и обнаружения узких
- **ComplexProcessor** [58% / 52%] - TODO: Refactor this complex method - intentional debt for quick delivery
- **featureextractortestts** [57% / 50%] - Тесты для Feature Extractor
- **testcriticalassessmentts** [57% / 51%] - Критическая проверка: работают ли новые компоненты изолированно
- **testrecommendationsimple** [57% / 50%] - Простой тест RecommendationEngine
- **testaiintegration** [57% / 50%] - EAP Analyzer компонент
- **testjestcleants** [57% / 50%] - Тест JestCheckerAdapter с SimpleOrchestrator
- **testphase4mvp** [57% / 49%] - Простой тест интеграции TestingChecker с AnalysisOrchestrator   Phase 4.2 MVP тестирование
- **testvitestcleants** [57% / 50%] - Чистый тест VitestCheckerAdapter с SimpleOrchestrator (без AI модулей)
- **testvitestsimplets** [57% / 50%] - Простой тест интеграции VitestCheckerAdapter с AnalysisOrchestrator
- **qualitypredictortestts** [57% / 50%] - Тесты для Quality Predictor - предсказателя качества кода
...
- **testallanalyzersfinal** [55% / 44%] - ИТОГОВЫЙ КОМПЛЕКСНЫЙ ТЕСТ ВСЕХ АНАЛИЗАТОРОВ   Phase 1 (Unit Testing) + Phase 2 (E2E Testing)
...
- **testsecurityfinal** [53% / 43%] - Тест SecurityChecker - CommonJS версия
